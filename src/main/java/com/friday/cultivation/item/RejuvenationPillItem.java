...
public class RejuvenationPillItem
extends PillItem {
    ...
    public ItemStack finishUsingItem(@NotNull ItemStack stack, @NotNull Level level, @NotNull LivingEntity user) {
        if (!level.isClientSide && user instanceof ServerPlayer) {
            ...
            if ((absorptionTicks = PillEffectSpecs.absorptionTicks(this, this.tier())) > 0) {
            ...
            player.displayClientMessage((Component)Component.translatable((String)"message.friday_cultivation.pill.rejuvenation.consumed", (Object[])new Object[]{this.tier().displayName(), (int)actualHealed}).withStyle(ChatFormatting.GREEN), true);
        }
    ...
    public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, @NotNull List<Component> tooltip, @NotNull TooltipFlag flag) {
        ...
        if (PillEffectSpecs.rejuvenationHealFull(this, this.tier())) {
            tooltip.add((Component)TooltipUtils.positiveLine((Component)Component.translatable((String)"tooltip.friday_cultivation.pill.rejuvenation.heal_full")));
        } else {
            int healAmount = Math.round(PillEffectSpecs.rejuvenationHeal(this, this.tier(), 0.0f));
            tooltip.add((Component)TooltipUtils.positiveLine((Component)Component.translatable((String)"tooltip.friday_cultivation.pill.rejuvenation.heal_amount", (Object[])new Object[]{healAmount})));
        }
        if (PillEffectSpecs.regenerationTicks(this, this.tier()) > 0 || PillEffectSpecs.absorptionTicks(this, this.tier()) > 0) {
            tooltip.add((Component)TooltipUtils.effectLine((Component)Component.translatable((String)"tooltip.friday_cultivation.pill.rejuvenation.supreme_buff")));
        }
        TooltipUtils.addBlank(tooltip);
