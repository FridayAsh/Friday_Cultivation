...
public class ExecuteCultivatorTradePacket {
    ...
    public static void handle(ExecuteCultivatorTradePacket msg, Supplier<NetworkEvent.Context> ctxSup) {
        ...
        ctx.enqueueWork(() -> {
            ...
            if (!(patt2648$temp instanceof WanderingCultivatorMenu) || (tradeMenu = (WanderingCultivatorMenu)patt2648$temp).getEntityId() != npc.getId()) {
                return;
            }
            ...
            MerchantOffer offer = (MerchantOffer)offers.get(msg.offerIndex);
            ...
            if (!ExecuteCultivatorTradePacket.playerHasItems(player.getInventory(), offer.getCostA()) || !offer.getCostB().isEmpty() && !ExecuteCultivatorTradePacket.playerHasItems(player.getInventory(), offer.getCostB())) {
                player.displayClientMessage((Component)Component.translatable((String)"message.friday_cultivation.cultivator.trade_no_qty"), true);
                return;
            }
            ...
            boolean isKnowledge = offer.getMaxUses() >= 2147482647;
            ...
            boolean bl = isSoulReaperTokenTrade = !isKnowledge && !npcHasResultInInventory && npc.isDifuReaper() && npc.isSoulReaperTokenTradeAvailable() && resultTemplate.is((Item)ModItems.SOUL_REAPER_TOKEN.get());
            if (!(isKnowledge || isHeldSwordTrade || isSoulReaperTokenTrade || npcHasResultInInventory)) {
                player.displayClientMessage((Component)Component.translatable((String)"message.friday_cultivation.cultivator.item_sold_out", (Object[])new Object[]{resultTemplate.getHoverName()}), true);
                return;
            }
    ...
    private static void consumeFromNpcInventory(WanderingCultivatorEntity npc, ItemStack result) {
        ...
        for (int i = 0; i < inv.getContainerSize() && toRemove > 0; ++i) {
            ItemStack slot = inv.getItem(i);
            if (!ItemStack.isSameItemSameTags((ItemStack)slot, (ItemStack)result)) continue;
            int takeFromSlot = Math.min(slot.getCount(), toRemove);
            slot.shrink(takeFromSlot);
            toRemove -= takeFromSlot;
        }
