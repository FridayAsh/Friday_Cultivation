    ...
    private static final class Task {
        ...
        private StepResult process(int visitBudget, int changeBudget) {
            ...
            while (!this.done && visited < visitBudget && changed < changeBudget) {
                ...
                pos.set(px, py, pz);
                if (!this.level.hasChunkAt((BlockPos)pos) || (state = this.level.getBlockState((BlockPos)pos)).isAir() || state.is(Blocks.BEDROCK) || state.is(Blocks.END_PORTAL_FRAME) || SectProtectionDomeHandler.isProtectedByAnySectProtectionDome((Level)this.level, (BlockPos)pos) || !SpellTerrainDestructionHelper.setBlock(this.level, (BlockPos)pos, air, 2, this.caster())) continue;
                ++changed;
            }
