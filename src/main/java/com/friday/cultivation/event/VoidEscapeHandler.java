...
public final class VoidEscapeHandler {
    ...
    public static void tickCharge(ServerPlayer player, CultivationData data) {
        ...
        if (data.getCurrentQi() < need) {
            data.clearCharging();
            player.displayClientMessage((Component)Component.translatable((String)"message.friday_cultivation.void_escape.charge_failed_qi"), true);
            CapabilityEvents.syncToClient(player);
            return;
        }
    ...
    private static void spawnChargeVisuals(ServerPlayer player, CultivationData data) {
        ...
        if (ticks == 1 || ticks % 20 == 0) {
            ...
            player.serverLevel().playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.PORTAL_AMBIENT, SoundSource.PLAYERS, 0.1f + progress * 0.1f, 0.7f + progress * 0.25f);
        }
    }
    ...
    public static void enter(ServerPlayer player, CultivationData data) {
        ...
        player.serverLevel().playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.CHORUS_FRUIT_TELEPORT, SoundSource.PLAYERS, 0.7f, 0.72f);
        ...
        CapabilityEvents.syncToClient(player);
        player.displayClientMessage((Component)Component.translatable((String)"message.friday_cultivation.void_escape.entered"), true);
    }
    ...
    public static void exit(ServerPlayer player, CultivationData data, ExitReason reason) {
        ...
        player.displayClientMessage((Component)Component.translatable((String)msgKey), true);
    }
    ...
    private static void applyVoidMovement(ServerPlayer player) {
        ...
        if (!tag.getBoolean(PHASE_APPLIED)) {
            ...
            tag.putBoolean(PREV_MAYFLY, player.getAbilities().mayfly);
            tag.putBoolean(PREV_FLYING, player.getAbilities().flying);
    ...
    private static void restoreVoidMovement(ServerPlayer player) {
        ...
        CultivationData data = CultivationCapability.get((Player)player).orElse(null);
        boolean passiveFlightAllowed = PassiveSpellHandler.hasEnabledPassiveFlight(data);
        ...
        if (player.isCreative()) {
            ...
            if (player.getAbilities().flying != restoredFlying) {
                player.getAbilities().flying = restoredFlying;
                dirty = true;
            }
        } else {
            if (player.getAbilities().mayfly != restoredMayfly) {
                player.getAbilities().mayfly = restoredMayfly;
                dirty = true;
            }
            if (player.getAbilities().flying != restoredFlying) {
                player.getAbilities().flying = restoredFlying;
