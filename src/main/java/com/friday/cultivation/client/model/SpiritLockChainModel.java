...
public final class SpiritLockChainModel {
    ...
    private static final RenderType CHAIN_RENDER_TYPE = RenderType.entityTranslucentEmissive((ResourceLocation)WHITE_TEXTURE);
    ...
    public static void renderEntity(PoseStack pose, MultiBufferSource buffers, float entityWidth, float entityHeight, float age, float alpha) {
        SpiritLockChainModel.renderEntity(pose, buffers, entityWidth, entityHeight, age, alpha, 16766042, 11103766, 0xFFF0A0);
    }
    ...
    public static void renderEntity(PoseStack pose, MultiBufferSource buffers, float entityWidth, float entityHeight, float age, float alpha, int mainColor, int darkColor, int lightColor) {
        ...
        float linkWidth = Mth.clamp((float)(entityWidth * 0.23f), (float)0.15f, (float)0.3f);
        ...
        int a = SpiritLockChainModel.alphaToByte(alpha);
        for (int strand = 0; strand < 3; ++strand) {
            ...
            for (int i = 0; i < 12; ++i) {
                ...
                SpiritLockChainModel.renderLinkAt(pose, vc, x, y, z, yaw, 0.38f, roll, linkWidth, linkHeight, linkDepth, thickness, a, mainColor, darkColor, lightColor);
            }
        }
        SpiritLockChainModel.renderEntityBelt(pose, vc, radius * 1.05f, height * 0.42f, age * 0.02f, linkWidth * 0.92f, linkHeight * 0.92f, linkDepth, thickness, a, mainColor, darkColor, lightColor);
        SpiritLockChainModel.renderEntityBelt(pose, vc, radius * 0.98f, height * 0.68f, -age * 0.018f + 0.7f, linkWidth * 0.86f, linkHeight * 0.86f, linkDepth, thickness, a, mainColor, darkColor, lightColor);
    }

    public static void renderBlock(PoseStack pose, MultiBufferSource buffers, float age, float alpha) {
        SpiritLockChainModel.renderBlock(pose, buffers, age, alpha, 16766042, 11103766, 0xFFF0A0);
    }
    ...
    public static void renderBlock(PoseStack pose, MultiBufferSource buffers, float age, float alpha, int mainColor, int darkColor, int lightColor) {
        ...
        for (int side = 0; side < 4; ++side) {
            ...
            for (int i = 0; i < 7; ++i) {
                ...
                SpiritLockChainModel.renderLinkAt(pose, vc, x, y, z, yaw, 0.18f, roll, linkWidth, linkHeight, linkDepth, thickness, a, mainColor, darkColor, lightColor);
            }
        }
        SpiritLockChainModel.renderBlockBelt(pose, vc, radius, -0.2f, age * 0.018f, linkWidth, linkHeight, linkDepth, thickness, a, mainColor, darkColor, lightColor);
        SpiritLockChainModel.renderBlockBelt(pose, vc, radius, 0.23f, -age * 0.018f + 0.35f, linkWidth, linkHeight, linkDepth, thickness, a, mainColor, darkColor, lightColor);
    }
    ...
    public static void renderChainBetween(PoseStack pose, MultiBufferSource buffers, Vec3 start, Vec3 end, float age, float alpha, int mainColor, int darkColor, int lightColor) {
        SpiritLockChainModel.renderChainBetween(pose, buffers, start, end, age, alpha, mainColor, darkColor, lightColor, 1.0f);
    }
    ...
    public static void renderChainBetween(PoseStack pose, MultiBufferSource buffers, Vec3 start, Vec3 end, float age, float alpha, int mainColor, int darkColor, int lightColor, float scale) {
        ...
        float pitch = (float)(-Math.asin(Mth.clamp((float)((float)dir.y), (float)-1.0f, (float)1.0f)));
        int links = Mth.clamp((int)((int)Math.ceil(length * 3.0 / (double)Math.max(0.6f, scale))), (int)4, (int)96);
        ...
        for (int i = 0; i <= links; ++i) {
            ...
            float roll = (i & 1) == 0 ? 0.0f : 1.5707964f;
            SpiritLockChainModel.renderLinkAt(pose, vc, (float)p.x, (float)p.y, (float)p.z, yaw, pitch, roll, linkWidth, linkHeight, linkDepth, thickness, a, mainColor, darkColor, lightColor);
        }
    }
    ...
    private static void renderEntityBelt(PoseStack pose, VertexConsumer vc, float radius, float y, float phase, float linkWidth, float linkHeight, float linkDepth, float thickness, int alpha, int mainColor, int darkColor, int lightColor) {
        for (int i = 0; i < 14; ++i) {
            ...
            SpiritLockChainModel.renderLinkAt(pose, vc, x, y, z, yaw, 0.0f, roll, linkWidth, linkHeight, linkDepth, thickness, alpha, mainColor, darkColor, lightColor);
        }
    }
    ...
    private static void renderLinkAt(PoseStack pose, VertexConsumer vc, float x, float y, float z, float yaw, float pitch, float roll, float width, float height, float depth, float thickness, int alpha, int mainColor, int darkColor, int lightColor) {
        ...
        SpiritLockChainModel.renderChainLink(pose, vc, width, height, depth, thickness, alpha, mainColor, darkColor, lightColor);
        pose.popPose();
    }
    ...
    private static void renderChainLink(PoseStack pose, VertexConsumer vc, float width, float height, float depth, float thickness, int alpha, int mainColor, int darkColor, int lightColor) {
        ...
        SpiritLockChainModel.box(pose, vc, -hw, hh - thickness, -hd, hw, hh, hd, lightColor, alpha);
        SpiritLockChainModel.box(pose, vc, -hw, -hh, -hd, hw, -hh + thickness, hd, darkColor, alpha);
        SpiritLockChainModel.box(pose, vc, -hw, -hh, -hd, -hw + sideThickness, hh, hd, mainColor, alpha);
        SpiritLockChainModel.box(pose, vc, hw - sideThickness, -hh, -hd, hw, hh, hd, mainColor, alpha);
    }
    ...
    private static void box(PoseStack pose, VertexConsumer vc, float minX, float minY, float minZ, float maxX, float maxY, float maxZ, int rgb, int alpha) {
        ...
        int g = rgb >> 8 & 0xFF;
        int b = rgb & 0xFF;
        SpiritLockChainModel.face(vc, m, n, minX, minY, maxZ, maxX, minY, maxZ, maxX, maxY, maxZ, minX, maxY, maxZ, 0.0f, 0.0f, 1.0f, r, g, b, alpha);
        SpiritLockChainModel.face(vc, m, n, maxX, minY, minZ, minX, minY, minZ, minX, maxY, minZ, maxX, maxY, minZ, 0.0f, 0.0f, -1.0f, r, g, b, alpha);
        SpiritLockChainModel.face(vc, m, n, minX, maxY, maxZ, maxX, maxY, maxZ, maxX, maxY, minZ, minX, maxY, minZ, 0.0f, 1.0f, 0.0f, r, g, b, alpha);
        SpiritLockChainModel.face(vc, m, n, minX, minY, minZ, maxX, minY, minZ, maxX, minY, maxZ, minX, minY, maxZ, 0.0f, -1.0f, 0.0f, r, g, b, alpha);
        SpiritLockChainModel.face(vc, m, n, maxX, minY, maxZ, maxX, minY, minZ, maxX, maxY, minZ, maxX, maxY, maxZ, 1.0f, 0.0f, 0.0f, r, g, b, alpha);
        SpiritLockChainModel.face(vc, m, n, minX, minY, minZ, minX, minY, maxZ, minX, maxY, maxZ, minX, maxY, minZ, -1.0f, 0.0f, 0.0f, r, g, b, alpha);
    }
    ...
    private static void face(VertexConsumer vc, Matrix4f m, Matrix3f n, float x1, float y1, float z1, float x2, float y2, float z2, float x3, float y3, float z3, float x4, float y4, float z4, float nx, float ny, float nz, int r, int g, int b, int a) {
        SpiritLockChainModel.vertex(vc, m, n, x1, y1, z1, 0.0f, 1.0f, nx, ny, nz, r, g, b, a);
        SpiritLockChainModel.vertex(vc, m, n, x2, y2, z2, 1.0f, 1.0f, nx, ny, nz, r, g, b, a);
        SpiritLockChainModel.vertex(vc, m, n, x3, y3, z3, 1.0f, 0.0f, nx, ny, nz, r, g, b, a);
        SpiritLockChainModel.vertex(vc, m, n, x4, y4, z4, 0.0f, 0.0f, nx, ny, nz, r, g, b, a);
    }
    ...
    private static void vertex(VertexConsumer vc, Matrix4f m, Matrix3f n, float x, float y, float z, float u, float v, float nx, float ny, float nz, int r, int g, int b, int a) {
        vc.vertex(m, x, y, z).color(r, g, b, a).uv(u, v).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(0xF000F0, 0xF000F0).normal(n, nx, ny, nz).endVertex();
    }
