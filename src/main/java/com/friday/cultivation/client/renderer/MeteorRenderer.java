...
extends EntityRenderer<MeteorEntity> {
    ...
    public void render(@NotNull MeteorEntity entity, float yaw, float partialTick, @NotNull PoseStack pose, @NotNull MultiBufferSource buf, int light) {
        ...
        VertexConsumer rockVc = buf.getBuffer(RenderType.entitySolid((ResourceLocation)METEOR_TEXTURE));
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (SphereVertex v : quad = sphereVertexArray[i]) {
                rockVc.vertex(m1, v.x, v.y, v.z).color(255, 255, 255, 255).uv(v.u, v.v).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(0xF000F0, 0xF000F0).normal(n1, v.x, v.y, v.z).endVertex();
            }
        }
        pose.popPose();
        pose.pushPose();
        ...
        VertexConsumer haloVc = buf.getBuffer(RenderType.entityTranslucentEmissive((ResourceLocation)HALO_TEXTURE));
        ...
        for (int i = 0; i < n3; ++i) {
            ...
            for (SphereVertex v : quad = sphereVertexArray2[i]) {
                haloVc.vertex(m2, v.x, v.y, v.z).color(rCol, gCol, bCol, alpha).uv(v.u, v.v).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(0xF000F0, 0xF000F0).normal(n2, v.x, v.y, v.z).endVertex();
            }
        }
        pose.popPose();
