...
extends EntityRenderer<BuddhaFireLotusEntity> {
    ...
    public static void renderLotus(PoseStack pose, MultiBufferSource buffers, int rootFlags, int chargedQi, float ageTicks, float scale, float alphaMultiplier, float rotationDegrees, float bloomProgress) {
        ...
        VertexConsumer vc = buffers.getBuffer(RenderType.entityTranslucentEmissive((ResourceLocation)WHITE));
    ...
    private static void emitTrailRibbon(VertexConsumer vc, Matrix4f matrix, Matrix3f normal, Vec3 forward, Vec3 right, Vec3 up, float ageTicks, float sideOffset, float width, float length, int color, int alpha) {
        double wave = (double)Mth.sin((float)(ageTicks * 0.3f)) * 0.24;
        ...
        Vec3 nearRight = nearCenter.multiply(right.scale((double)width));
        Vec3 farLeft = farCenter.add(right.scale((double)width * 0.25));
        Vec3 farRight = farCenter.multiply(right.scale((double)width * 0.25));
    ...
    private static float[] lightningPoint(float angle, float radius, float y, int seed, int segment, float radiusJitter, float yJitter) {
        ...
        return new float[]{Mth.cos((float)angle) * r, y + BuddhaFireLotusRenderer.signedHash(seed + segment * 311) * yJitter, Mth.sin((float)angle) * r};
    }
    ...
    private static void emitLightningSegment(VertexConsumer vc, Matrix4f matrix, Matrix3f normal, float[] a, float[] b, float thickness, int color, int alpha) {
        ...
        BuddhaFireLotusRenderer.quad(vc, matrix, normal, BuddhaFireLotusRenderer.vec(start.add(s)), BuddhaFireLotusRenderer.vec(end.add(s)), BuddhaFireLotusRenderer.vec(end.multiply(s)), BuddhaFireLotusRenderer.vec(start.multiply(s)), color, Mth.clamp((int)alpha, (int)0, (int)255));
        BuddhaFireLotusRenderer.quad(vc, matrix, normal, BuddhaFireLotusRenderer.vec(start.add(u)), BuddhaFireLotusRenderer.vec(end.add(u)), BuddhaFireLotusRenderer.vec(end.multiply(u)), BuddhaFireLotusRenderer.vec(start.multiply(u)), color, Mth.clamp((int)((int)((float)alpha * 0.72f)), (int)0, (int)255));
    }
    ...
    private static void emitCyanShockwaveBand(VertexConsumer vc, Matrix4f matrix, Matrix3f normal, int segments, float radius, float y, float thickness, int alpha) {
        ...
        for (int i = 0; i < segments; ++i) {
            float a0 = (float)(Math.PI * 2 * (double)i / (double)segments);
            float a1 = (float)(Math.PI * 2 * (double)(i + 1) / (double)segments);
            float wave0 = Mth.sin((float)(a0 * 4.0f)) * thickness * 0.16f;
            float wave1 = Mth.sin((float)(a1 * 4.0f)) * thickness * 0.16f;
            float[] inner0 = new float[]{Mth.cos((float)a0) * inner, y + wave0, Mth.sin((float)a0) * inner};
            float[] outer0 = new float[]{Mth.cos((float)a0) * outer, y + wave0 + thickness * 0.1f, Mth.sin((float)a0) * outer};
            float[] outer1 = new float[]{Mth.cos((float)a1) * outer, y + wave1 + thickness * 0.1f, Mth.sin((float)a1) * outer};
            float[] inner1 = new float[]{Mth.cos((float)a1) * inner, y + wave1, Mth.sin((float)a1) * inner};
    ...
    private static void vertex(VertexConsumer vc, Matrix4f matrix, Matrix3f normal, float x, float y, float z, int color, int alpha) {
        vc.vertex(matrix, x, y, z).color(color >> 16 & 0xFF, color >> 8 & 0xFF, color & 0xFF, alpha).uv(0.0f, 0.0f).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(0xF000F0, 0xF000F0).normal(normal, 0.0f, 1.0f, 0.0f).endVertex();
    }
