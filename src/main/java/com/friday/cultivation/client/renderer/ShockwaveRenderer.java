...
extends EntityRenderer<ShockwaveEntity> {
    ...
    public void render(@NotNull ShockwaveEntity entity, float yaw, float partialTick, @NotNull PoseStack pose, @NotNull MultiBufferSource buf, int light) {
        ...
        VertexConsumer vc = buf.getBuffer(RenderType.entityTranslucentEmissive((ResourceLocation)WHITE_TEXTURE));
        ...
        for (int i = 0; i < n2; ++i) {
            ...
            for (SphereVertex v : quad = sphereVertexArray[i]) {
                vc.vertex(m, v.x, v.y, v.z).color(255, 255, 255, alphaByte).uv(v.u, v.v).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(0xF000F0, 0xF000F0).normal(n, v.x, v.y, v.z).endVertex();
            }
        }
