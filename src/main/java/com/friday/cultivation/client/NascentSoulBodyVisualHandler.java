...
public final class NascentSoulBodyVisualHandler {
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        while (it.hasNext()) {
            ...
            if (now < entry.getValue().expiresAtMs() && mc.level.getEntity(entry.getKey().intValue()) != null) continue;
            it.remove();
        }
    }
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        for (Map.Entry<Integer, BodyVisual> entry : BODIES.entrySet()) {
            ...
            if (visual.pos().distanceToSqr(camera) <= 36864.0) {
                NascentSoulBodyVisualHandler.renderBodyProjection(mc, pose, (MultiBufferSource)buffers, player, visual, camera, partial);
            }
            NascentSoulBodyVisualHandler.renderSoulProjection(mc, pose, (MultiBufferSource)buffers, player, camera, partial);
        }
        buffers.endBatch();
    }
    ...
    private static void renderPlayerProjection(Minecraft mc, PoseStack pose, MultiBufferSource buffers, AbstractClientPlayer player, Vec3 pos, float yRot, float xRot, Vec3 camera, float partial, float red, float green, float blue, float alpha) {
        ...
        NascentSoulBodyVisualHandler.configureBodyModel((PlayerModel<AbstractClientPlayer>)model, player);
        pose.pushPose();
        ...
        VertexConsumer buffer = alpha >= 0.99f ? buffers.getBuffer(model.renderType(player.getSkinTextureLocation())) : buffers.getBuffer(RenderType.entityTranslucent((ResourceLocation)player.getSkinTextureLocation()));
