...
public final class TimeStasisClientEffects {
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        SINGLE_TARGETS.entrySet().removeIf(entry -> now >= (Long)entry.getValue());
        TimeStasisClientEffects.applyClientEntityFreeze(mc);
        CLIENT_FROZEN.entrySet().removeIf(entry -> mc.level.getEntity(((Integer)entry.getKey()).intValue()) == null);
    ...
    public static void onRenderLivingPre(RenderLivingEvent.Pre<?, ?> event) {
        ...
        RenderSystem.setShaderColor((float)0.62f, (float)0.62f, (float)0.62f, (float)1.0f);
        tintingEntity = true;
    }
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        for (ActiveDomain domain : DOMAINS) {
            ...
            expand = TimeStasisClientEffects.domainExpansion(domain, now);
            radius = domain.radius() * expand;
            alpha = (float)(0.16 + 0.08 * expand);
        ...
        for (ActiveDomain domain : DOMAINS) {
            ...
            expand = TimeStasisClientEffects.domainExpansion(domain, now);
            radius = domain.radius() * expand;
            alpha = (float)(0.45 + 0.12 * (1.0 - expand));
    ...
    private static void renderOverlay(GuiGraphics graphics, int screenWidth, int screenHeight) {
        ...
        boolean canAct = TimeStasisClientEffects.canActInStoppedTime((Entity)player);
        boolean insideDomain = TimeStasisClientEffects.isInsideExpandedDomain(player.position());
        boolean bl = frozen = !canAct && (TimeStasisClientEffects.isSingleTargetFrozen(player.getId()) || insideDomain);
    ...
    private static boolean shouldTint(LivingEntity entity) {
        return !TimeStasisClientEffects.canActInStoppedTime((Entity)entity) && (TimeStasisClientEffects.isSingleTargetFrozen(entity.getId()) || TimeStasisClientEffects.isInsideExpandedDomain(entity.position()));
    }

    private static boolean isLocalPlayerVisuallyFrozen(LocalPlayer player) {
        return !TimeStasisClientEffects.canActInStoppedTime((Entity)player) && (TimeStasisClientEffects.isSingleTargetFrozen(player.getId()) || TimeStasisClientEffects.isInsideExpandedDomain(player.position()));
    }

    private static boolean shouldBlockLocalPlayer(LocalPlayer player) {
        return !TimeStasisClientEffects.canActInStoppedTime((Entity)player) && (TimeStasisClientEffects.isSingleTargetFrozen(player.getId()) || TimeStasisClientEffects.isInsideAnyDomain(player.position()));
    }

    private static boolean canActInStoppedTime(Entity entity) {
        LivingEntity living;
        LocalPlayer player;
        if (entity instanceof LocalPlayer && (player = (LocalPlayer)entity).isSpectator()) {
            return true;
        }
        if (TimeStasisClientEffects.isDomainCaster(entity.getId())) {
            return true;
        }
        return entity instanceof LivingEntity && (living = (LivingEntity)entity).hasEffect((MobEffect)ModEffects.TIME_STASIS_FLOW.get());
    }
    ...
    private static void updateDesaturateShader(Minecraft mc) {
        boolean shouldUseShader;
        boolean bl = shouldUseShader = mc.player != null && (TimeStasisClientEffects.isLocalPlayerVisuallyFrozen(mc.player) || TimeStasisClientEffects.isInsideExpandedDomain(mc.player.position()));
    ...
    private static void freezeClientEntity(Entity entity) {
        ...
        entity.xOld = snapshot.position().x;
        entity.yOld = snapshot.position().y;
        entity.zOld = snapshot.position().z;
    ...
    private static void renderDomainCountdown(GuiGraphics graphics, LocalPlayer player, int screenWidth) {
        ...
        MutableComponent text = Component.translatable((String)"hud.friday_cultivation.time_stasis.remaining", (Object[])new Object[]{String.format(Locale.ROOT, "%.1f", remainingSeconds)});
        Minecraft mc = Minecraft.getInstance();
        int width = mc.font.width((FormattedText)text);
        graphics.drawString(mc.font, (Component)text, (screenWidth - width) / 2, 28, 14543103, true);
    }
