...
public final class ChargingPreviewHandler {
    ...
    public static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        if (charge.spell() == Spell.GREAT_FIREBALL) {
            ...
            FireballRenderHelper.render(pose, (MultiBufferSource)buffers, scale, age);
            buffers.endBatch();
            ...
            HeavenPiercingConeRenderer.renderCone(pose, (MultiBufferSource)buffers, look, stage, charge.chargingTicks(), (float)charge.chargingTicks() + partial, 0.9f);
            buffers.endBatch();
    ...
    public static void onRenderHand(RenderHandEvent event) {
        ...
        Minecraft mc = Minecraft.getInstance();
        LocalPlayer player = mc.player;
        if (player == null || mc.level == null || player.isInvisible()) {
            return;
        }
    ...
    private static void renderCoreSelfDestructWorld(Minecraft mc, PoseStack pose, Vec3 camPos, LocalPlayer player, LocalCharge charge, float partial) {
        ...
        double y = Mth.lerp((double)partial, (double)player.yOld, (double)player.getY()) + (double)player.getBbHeight() * 0.58;
    ...
    private static void addDisc(BufferBuilder buffer, Matrix4f mat, float radius, int color, float alpha, int segments, float rotation) {
        ...
        int g = color >> 8 & 0xFF;
        int b = color & 0xFF;
        int a = (int)(Mth.clamp((float)alpha, (float)0.0f, (float)1.0f) * 255.0f);
    ...
    private static void addCoreRays(BufferBuilder buffer, Matrix4f mat, float radius, float progress, float age, float rayScale) {
        ...
        RenderSystem.lineWidth((float)(1.5f + progress * 2.0f));
    ...
    private static void renderBuddhaFireLotusPreview(Minecraft mc, PoseStack pose, Vec3 camPos, Vec3 eye, Vec3 look, LocalCharge charge, float partial) {
        ...
        if (progress >= 0.88f) {
            ...
            ChargingPreviewHandler.renderPreviewLotus(pose, (MultiBufferSource)buffers, camPos, center, 42, charge.chargedQi(), age, scale, alpha, formed);
        }
        buffers.endBatch();
    }
    ...
    private static LivingEntity findBuddhaFireLotusTarget(Minecraft mc, LocalPlayer player, float partial) {
        ...
        for (LivingEntity entity : mc.level.getEntitiesOfClass(LivingEntity.class, scan, e -> e != player && e.isAlive() && e.isPickable())) {
            ...
            if (hit.isEmpty() || !((dist = eye.distanceTo((Vec3)hit.get())) <= blockDist + 0.35) || !(dist < bestDist)) continue;
            bestDist = dist;
            best = entity;
        }
        return best;
    ...
    private static void renderBuddhaFireLotusTargetOutline(Minecraft mc, PoseStack pose, Vec3 camPos, float partial) {
        ...
        double x = Mth.lerp((double)partial, (double)target.xOld, (double)target.getX());
        double y = Mth.lerp((double)partial, (double)target.yOld, (double)target.getY());
        double z = Mth.lerp((double)partial, (double)target.zOld, (double)target.getZ());
    ...
    private static void renderClaspingArm(PoseStack pose, MultiBufferSource buffers, int packedLight, PlayerRenderer playerRenderer, LocalPlayer player, HumanoidArm arm, float progress) {
        ...
        float arcDegrees = Mth.lerp((float)reveal, (float)Mth.lerp((float)approach, (float)0.0f, (float)58.0f), (float)34.0f);
        float palmYaw = Mth.lerp((float)reveal, (float)Mth.lerp((float)approach, (float)45.0f, (float)112.0f), (float)88.0f);
        float palmPitch = Mth.lerp((float)reveal, (float)Mth.lerp((float)approach, (float)0.0f, (float)28.0f), (float)14.0f);
        float wristRoll = Mth.lerp((float)reveal, (float)Mth.lerp((float)approach, (float)0.0f, (float)16.0f), (float)8.0f);
        pose.pushPose();
        pose.translate(shoulderX, shoulderY, shoulderZ);
        pose.mulPose(Axis.ZP.rotationDegrees(side * arcDegrees));
        pose.mulPose(Axis.YP.rotationDegrees(side * palmYaw));
        pose.mulPose(Axis.XP.rotationDegrees(palmPitch));
        pose.mulPose(Axis.ZP.rotationDegrees(side * wristRoll));
    ...
    private static void spawnFireballChargeParticles(Minecraft mc, LocalPlayer player, int chargedQi, Vec3 eye, Vec3 look) {
        ...
        for (int i = 0; i < sparkCount; ++i) {
            ...
            mc.level.addParticle((ParticleOptions)particle, center.x + sx * (double)visualR, center.y + sy * (double)visualR, center.z + sz * (double)visualR, sx * speed, sy * speed + 0.02, sz * speed);
        }
    }
    ...
    private static void spawnBuddhaFireLotusParticles(Minecraft mc, int chargedQi, Vec3 center, Vec3 look) {
        ...
        Vec3 left = handBase.multiply(right.scale(handOffset));
    ...
    private static void spawnPalmFireCluster(Minecraft mc, Vec3 origin, Vec3 target, Vec3 right, Vec3 look, DustParticleOptions dust, ParticleOptions flame, float progress, boolean cyan) {
        ...
        Vec3 inward = target.multiply(origin).scale(pull);
        for (int i = 0; i < 3; ++i) {
