...
public final class DeathSequenceClientEffects {
    ...
    private static void render(GuiGraphics g, int sw, int sh) {
        ...
        if (elapsed < titleTicks) {
            ...
            DeathSequenceClientEffects.drawScaledCentered(g, font, (Component)Component.translatable((String)"screen.friday_cultivation.death.title"), cx, titleY, 3.4f, -4048338);
            DeathSequenceClientEffects.drawCentered(g, font, (Component)Component.translatable((String)"screen.friday_cultivation.death.desc1"), cx, titleY + 40, -1648452);
            DeathSequenceClientEffects.drawCentered(g, font, (Component)Component.translatable((String)"screen.friday_cultivation.death.desc2"), cx, titleY + 52, -1648452);
        } else {
            ...
            DeathSequenceClientEffects.drawCentered(g, font, (Component)Component.translatable((String)"screen.friday_cultivation.death.countdown"), cx, y, -1648452);
            float pulse = sec <= 3 ? 2.4f : 1.9f;
            int numColor = sec <= 3 ? -4048338 : -15797;
            DeathSequenceClientEffects.drawScaledCentered(g, font, (Component)Component.literal((String)String.valueOf(sec)), cx, y + 13, pulse, numColor);
        }
    }

    private static void drawCentered(GuiGraphics g, Font font, Component text, int cx, int y, int color) {
        g.drawString(font, text, cx - font.width((FormattedText)text) / 2, y, color, true);
    }
