...
public final class RealmPressureClientEffects {
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        SUPPRESSED_TARGETS.entrySet().removeIf(entry -> (Long)entry.getValue() <= now);
        CASTER_HALOS.entrySet().removeIf(entry -> (Long)entry.getValue() <= now);
        EXPANSIONS.entrySet().removeIf(entry -> ((ExpansionVisual)entry.getValue()).endMs <= now);
    }
    ...
    public static void onRenderLiving(RenderLivingEvent.Post<?, ?> event) {
        ...
        if (haloEnd > now) {
            ...
            float alpha = RealmPressureClientEffects.alpha(haloEnd, 600L) * (0.2f + 0.07f * Mth.sin((float)(age * 0.11f)));
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        for (Map.Entry<Integer, ExpansionVisual> entry : EXPANSIONS.entrySet()) {
            ...
            if (!(entity instanceof LivingEntity) || !(caster = (LivingEntity)entity).isAlive() || mc.player.distanceToSqr((Entity)caster) > 25600.0) continue;
            double radius = Math.max(1.25, (double)visual.radius);
            Vec3 center = caster.getPosition(event.getPartialTick()).add(0.0, (double)caster.getBbHeight() * 0.55, 0.0);
    ...
    private static void renderHaloSphere(PoseStack pose, MultiBufferSource buffers, float radius, float alpha) {
        ...
        Matrix4f mat = pose.last().pose();
        int a = Mth.clamp((int)((int)(alpha * 255.0f)), (int)0, (int)255);
    ...
    private static void renderDescendingRunes(PoseStack pose, MultiBufferSource buffers, LivingEntity entity, float age, float alpha) {
        ...
        int baseAlpha = Mth.clamp((int)((int)(alpha * 185.0f)), (int)0, (int)185);
        for (int strand = 0; strand < 5; ++strand) {
            ...
            for (int i = 0; i < 6; ++i) {
                ...
                float terminal = y <= 0.16f ? Mth.clamp((float)((0.16f - y) / 0.48f), (float)0.0f, (float)1.0f) : 0.0f;
                float terminalEase = RealmPressureClientEffects.smooth(terminal);
                float outward = terminal > 0.0f ? 0.1f + terminalEase * 0.38f : 0.0f;
                float x = Mth.cos((float)angle) * (radius + outward);
                float z = Mth.sin((float)angle) * (radius + outward);
                float flicker = 0.72f + 0.28f * Mth.sin((float)(age * 0.21f + (float)i * 1.7f + (float)strand));
                int a = Mth.clamp((int)((int)((float)baseAlpha * flicker * (fade = terminal > 0.0f ? 1.0f - terminalEase * 0.88f : 1.0f))), (int)0, (int)220);
                if (a <= 4) continue;
                float glyphWidth = 0.15f + entity.getBbWidth() * 0.06f;
                float glyphHeight = 0.25f + entity.getBbHeight() * 0.032f;
                if (terminal > 0.0f) {
                    RealmPressureClientEffects.drawGroundRuneGlyph(pose, vc, x, Mth.lerp((float)terminalEase, (float)0.16f, (float)0.035f), z, angle + 1.5707964f, glyphWidth * (1.05f + terminalEase * 0.45f), glyphHeight * (0.82f + terminalEase * 0.2f), i + strand, a);
                    continue;
                }
    ...
    private static void drawSealGlyph(VertexConsumer vc, Matrix4f mat, float width, float height, int variant, int alpha) {
        ...
        int frameAlpha = Mth.clamp((int)((int)((float)alpha * 0.82f)), (int)0, (int)255);
        ...
        RealmPressureClientEffects.rect(vc, mat, -stroke * 0.95f, -stroke * 0.95f, stroke * 0.95f, stroke * 0.95f, 16740936, Mth.clamp((int)((int)((float)alpha * 0.92f)), (int)0, (int)255));
    }
    ...
    private static void stroke(VertexConsumer vc, Matrix4f mat, float x1, float y1, float x2, float y2, float thickness, int color, int alpha) {
        float dx = x2 - x1;
        float dy = y2 - y1;
        float len = Mth.sqrt((float)(dx * dx + dy * dy));
        if (len <= 1.0E-4f) {
            return;
        }
    ...
    private static float[][] buildSphereVertices() {
        ...
        for (int stack = 0; stack < 12; ++stack) {
            ...
            for (int sector = 0; sector < 24; ++sector) {
                ...
                float c1 = Mth.cos((float)t1);
                float s1 = Mth.sin((float)t1);
                float c2 = Mth.cos((float)t2);
                float s2 = Mth.sin((float)t2);
    ...
    private static float alpha(long endMs, long fadeMs) {
        ...
        if (remaining < fadeMs) {
            return Mth.clamp((float)((float)remaining / (float)fadeMs), (float)0.0f, (float)1.0f);
        }
        return 1.0f;
    }
