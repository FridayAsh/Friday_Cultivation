...
extends AbstractContainerMenu {
    ...
    public AlchemyMenu(int containerId, Inventory inv, BlockPos corePos) {
        super((MenuType)ModMenuTypes.ALCHEMY.get(), containerId);
        AlchemyCoreBlockEntity ace;
        this.corePos = corePos;
        this.player = inv.player;
        BlockEntity be = inv.player.level().getBlockEntity(corePos);
        this.blockEntityRef = be instanceof AlchemyCoreBlockEntity ? (ace = (AlchemyCoreBlockEntity)be) : null;
    ...
    public boolean onTake(@NotNull Player player) {
        ...
        if (!AlchemyFurnaceStructure.isComplete((LevelReader)player.level(), this.corePos)) {
            return false;
        }
