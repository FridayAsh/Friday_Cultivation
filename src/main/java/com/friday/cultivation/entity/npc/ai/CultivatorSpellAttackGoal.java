...
public class CultivatorSpellAttackGoal
extends Goal {
    ...
    public void start() {
        ...
        boolean lowHp;
        LivingEntity target = this.npc.getTarget();
        boolean bl = lowHp = this.npc.getHealth() < this.npc.getMaxHealth() * 0.5f;
    ...
    private Spell pickChargeable() {
        ...
        ArrayList<Spell> chargeables = new ArrayList<Spell>();
        for (String id : this.npc.getSpellIds()) {
            Spell s = Spell.byId(id);
            if (s == null || !NpcSpellCaster.CHARGEABLE_SUPPORTED.contains((Object)s) || qi < NpcSpellCaster.megaCost(this.npc, s) || NpcSpellCaster.isHighImpactMega(s) && (this.highImpactMegaCooldown > 0 || NpcSpellCaster.hasNearbyHighImpactSpell(this.npc, 160.0))) continue;
            chargeables.add(s);
        }
        if (chargeables.isEmpty()) {
            return null;
        }
        return (Spell)((Object)chargeables.get(this.npc.getRandom().nextInt(chargeables.size())));
    }

    private Spell pickAffordable(List<Spell> candidates) {
        ArrayList<Spell> affordable = new ArrayList<Spell>();
        long qi = this.npc.getCurrentQi();
        ...
        return (Spell)((Object)affordable.get(this.npc.getRandom().nextInt(affordable.size())));
    }
    ...
    private List<Spell> getCastableLearned(boolean selfBuffOnly) {
        ...
        for (String id : this.npc.getSpellIds()) {
            Spell s = Spell.byId(id);
            if (s == null || !NpcSpellCaster.SUPPORTED.contains((Object)s)) continue;
            if (selfBuffOnly) {
                if (!NpcSpellCaster.SELF_BUFF.contains((Object)s)) continue;
                result.add(s);
                continue;
            }
    ...
    private boolean hasAnyCastableSpell() {
        for (String id : this.npc.getSpellIds()) {
            Spell s = Spell.byId(id);
            if (s == null || !NpcSpellCaster.isCombatSpell(s) && !NpcSpellCaster.SELF_BUFF.contains((Object)s)) continue;
            return true;
        }
        return false;
    }
    ...
    private boolean hasAnyAffordableSelfBuff() {
        ...
        for (String id : this.npc.getSpellIds()) {
            Spell s = Spell.byId(id);
            if (s == null || !NpcSpellCaster.SELF_BUFF.contains((Object)s) || NpcSpellCaster.spellCost(this.npc, s) > qi) continue;
            return true;
        }
        return false;
    }
