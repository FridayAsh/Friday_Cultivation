...
extends Entity {
    ...
    public IceShellEntity(ServerLevel level, LivingEntity target) {
        this((EntityType<? extends IceShellEntity>)((EntityType)ModEntities.ICE_SHELL.get()), (Level)level);
    ...
    public void tick() {
        ...
        if (level instanceof ServerLevel) {
            ...
            this.level().addParticle((ParticleOptions)ParticleTypes.SNOWFLAKE, target.getX() + (this.random.nextDouble() - 0.5), target.getY() + this.random.nextDouble() * (double)target.getBbHeight(), target.getZ() + (this.random.nextDouble() - 0.5), 0.0, 0.05, 0.0);
        }
    }
    ...
    private void releaseTarget(LivingEntity target) {
        ...
        if (level instanceof ServerLevel) {
            ...
            for (int i = 0; i < 30; ++i) {
                ...
                double dy = this.random.nextDouble() * (double)target.getBbHeight();
                double dz = (this.random.nextDouble() - 0.5) * 1.5;
                server.sendParticles((ParticleOptions)ParticleTypes.SNOWFLAKE, target.getX() + dx, target.getY() + dy, target.getZ() + dz, 1, dx * 0.2, dy * 0.05, dz * 0.2, 0.1);
            }
        }
    }
    ...
    private void spawnFreezeParticles(ServerLevel server, LivingEntity target) {
        for (int i = 0; i < 6; ++i) {
            ...
            double y = this.random.nextDouble() * (double)target.getBbHeight();
            double r = (double)target.getBbWidth() * 0.7;
            double px = target.getX() + Math.cos(angle) * r;
            double pz = target.getZ() + Math.sin(angle) * r;
            server.sendParticles((ParticleOptions)ParticleTypes.SNOWFLAKE, px, target.getY() + y, pz, 1, 0.0, 0.02, 0.0, 0.05);
        }
    }
    ...
    private LivingEntity findTarget() {
        ...
        for (Entity e : this.level().getEntities((Entity)this, this.getBoundingBox().inflate(2.0))) {
    ...
    protected AABB makeBoundingBox() {
        return new AABB(this.getX() - 1.5, this.getY() - 0.5, this.getZ() - 1.5, this.getX() + 1.5, this.getY() + 2.5, this.getZ() + 1.5);
    }
}
