...
public final class SpellScalingHelper {
    ...
    public static double playerDamageMultiplier(Player player, Spell spell) {
        ...
        multiplier *= WeaponBonusHelper.spellDamageMultiplier((LivingEntity)player, spell);
        ...
        multiplier *= SpellScalingHelper.formationSpellDamageMultiplier((LivingEntity)player, spell);
        if (player instanceof ServerPlayer) {
            ServerPlayer serverPlayer = (ServerPlayer)player;
            multiplier *= DharmaBodyManifestationHandler.spellDamageMultiplier(serverPlayer);
        }
        return SpellScalingHelper.sanitizeMultiplier(multiplier *= RealmPressureHandler.outgoingDamageMultiplier((LivingEntity)player));
    }
    ...
    public static double npcDamageMultiplier(WanderingCultivatorEntity npc, Spell spell) {
        ...
        multiplier *= WeaponBonusHelper.spellDamageMultiplier((LivingEntity)npc, spell);
        multiplier *= SpellScalingHelper.formationSpellDamageMultiplier((LivingEntity)npc, spell);
        multiplier *= DharmaBodyManifestationHandler.spellDamageMultiplier(npc);
        if (npc.isNpcSoulState()) {
            multiplier *= 0.1;
        }
        return SpellScalingHelper.sanitizeMultiplier(multiplier *= RealmPressureHandler.outgoingDamageMultiplier((LivingEntity)npc));
    }
    ...
    public static int damageBonus(Player player, Spell spell) {
        ...
        return SpellScalingHelper.scaledDamage((LivingEntity)player, spell) - spell.damage();
    }
    ...
    private static double npcSpellElementMultiplier(WanderingCultivatorEntity npc, SpiritRoot root, QiElement element) {
        ...
        if (bonus.environmentBuff() && bonus.primaryElement() == element) {
            multiplier = SpellScalingHelper.applyEnvironmentBuff((LivingEntity)npc, element, multiplier);
        }
        return multiplier;
    }
