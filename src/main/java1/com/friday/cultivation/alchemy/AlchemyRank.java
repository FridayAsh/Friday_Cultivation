...
public enum AlchemyRank {
    ...
    AlchemyRank(String id, ChatFormatting color, int xpToNext, int[] probabilities) {
        this.id = id;
        this.color = color;
    ...
    public Component displayName() {
        return Component.translatable("alchemy_rank.friday_cultivation." + this.id);
    }
    ...
    public AlchemyRank next() {
        AlchemyRank[] vals = AlchemyRank.values();
        int idx = this.ordinal();
        return idx + 1 < vals.length ? vals[idx + 1] : this;
    }

    /** 掷骰决定炼丹产物品阶（照搬原模组：i==0 表示失败返回 null） */
    public PillTier rollPillResult(RandomSource rng) {
        ...
        for (int i = 0; i < this.probabilities.length; ++i) {
            ...
            return PillTier.values()[i - 1];
        }
        return null;
    }

    public static int xpGainFor(PillTier tier) {
        return switch (tier) {
            case LOW -> 5;
            case MID -> 15;
            case HIGH -> 50;
    ...
    public static AlchemyRank byId(String id) {
        for (AlchemyRank r : AlchemyRank.values()) {
    ...
    /** 根据炼丹熟练度(xp)获取对应等级（项目兼容方法，供 CultivationData 使用） */
    public static AlchemyRank fromXp(int xp) {
        AlchemyRank rank = APPRENTICE;
        int threshold = 0;
        for (AlchemyRank r : values()) {
            if (r == IMMORTAL) continue;
            if (xp >= threshold) rank = r;
            threshold += r.xpToNext;
        }
        return rank;
    }
}
