...
/**
 * 宗门屏 - 严格 1:1 复刻原模组 com.xiaoxiang.cultivation.client.screen.SectScreen
 */
...
extends Screen {
    ...
    private void toggleImmunity() {
        ModNetwork.CHANNEL.sendToServer(new SetSectFriendlyFirePacket(!this.sameSectImmunity, this.targetEntityId));
    ...
    private void renderSectPage(GuiGraphics gfx, int mx, int my) {
        ...
        Component titleLine = hasSect ? Component.literal(this.snapshot.getString("sectName")) : SectScreen.tr("none");
        ...
        int h = 316 - top;
        switch (this.tab) {
            case INFO -> this.renderInfoTab(gfx, top, h, mx, my);
            case MEMBERS -> this.renderMembersTab(gfx, top, h, mx, my);
            case TREE -> this.renderRelationsTab(gfx, top, h, mx, my);
            case TASKS -> this.renderTasksTab(gfx, top, h, mx, my);
        }
    ...
    private void renderInfoTab(GuiGraphics gfx, int y, int h, int mx, int my) {
        ...
        int sealCx = lx + 26;
        int sealCy = y + 26;
        this.sealCircle(gfx, sealCx, sealCy, 24, this.headerAccentColor(), Component.literal(SectScreen.firstGlyph(this.snapshot.getString("sectName"))));
        int nx = lx + 58;
        this.drawScaledString(gfx, Component.literal(this.snapshot.getString("sectName")).withStyle(ChatFormatting.BOLD), nx, y + 8, 1.35f, -14081252, 229);
        Component roleLine = this.viewerEnemy ? SectScreen.tr("viewer_hostile") : (this.viewerMember ? this.viewerRole.displayName() : SectScreen.tr("summary.guest"));
        this.drawTagPill(gfx, nx, y + 30, roleLine, this.viewerEnemy ? -7723482 : -7705298);
        int cy = y + 56;
        this.drawDiamondHeader(gfx, lx, cy, 289, SectScreen.tr("overview"));
        this.drawStatGridRow(gfx, lx, cy += 16, 289, SectScreen.tr("summary.members"), Component.literal(String.valueOf(this.members.size())), -13668780, SectScreen.tr("summary.tasks"), Component.literal(String.valueOf(this.activeTaskCount())), -13668470);
        this.drawStatGridRow(gfx, lx, cy += 14, 289, SectScreen.tr("summary.contribution"), Component.literal(String.valueOf(this.viewerMember ? this.viewerContribution : 0)), this.viewerMember ? -4885446 : -7700107, SectScreen.tr("book.ready"), Component.literal(String.valueOf(this.readyTaskCount())), this.readyTaskCount() > 0 ? -5750484 : -9807288);
        this.drawStatGridRow(gfx, lx, cy += 14, 289, SectScreen.tr("immunity"), SectScreen.tr(this.sameSectImmunity ? "book.on" : "book.off"), this.sameSectImmunity ? -13668780 : -5750484, SectScreen.tr("book.relation"), this.relationStatus(), this.relationColor());
        ...
        if (master != null) {
            ...
            this.drawScaledString(gfx, Component.literal(master.name).withStyle(ChatFormatting.BOLD), lx + 24, cy + 4, 0.95f, -14081252, 193);
            this.drawScaledRight(gfx, SectScreen.tr("member_meta_contribution", master.contribution), lx + 289 - 8, cy + 5, 0.75f, -7723482);
            this.drawScaledString(gfx, SectScreen.tr("book.leader_sub"), lx + 24, cy + 16, 0.74f, -9807288, 259);
        } else {
            this.drawScaledString(gfx, SectScreen.tr("book.no_leader"), lx + 6, cy + 4, 0.78f, -9807288, 277);
        }
        this.drawDiamondHeader(gfx, lx, cy += 34, 289, SectScreen.tr("book.notice"));
        this.drawWrappedLimited(gfx, this.infoNotice(), lx, cy += 15, 289, 0.78f, -11911632, 3);
        int rx = 329;
        int cy2 = y;
        this.drawDiamondHeader(gfx, rx, cy2, 289, SectScreen.tr("book.role_dist"));
        cy2 += 16;
        cy2 = this.renderRoleDistribution(gfx, rx, cy2, 289, mx, my);
        this.drawPaperDivider(gfx, rx + 6, cy2 += 6, 277);
        this.drawDiamondHeader(gfx, rx, cy2 += 10, 289, SectScreen.tr("book.recent_tasks"));
    ...
    private int renderRoleDistribution(GuiGraphics gfx, int x, int y, int w, int mx, int my) {
        ...
        for (int i = 0; i < roles.length; ++i) {
            ...
            if (total <= 0) {
                ...
                segW = Math.max(counts[i] > 0 ? 2 : 0, w * counts[i] / Math.max(1, total));
            }
            if (segW <= 0) continue;
        ...
        for (int i = 0; i < roles.length; ++i) {
            ...
            this.drawScaledRight(gfx, Component.literal("×" + counts[i]), ex + colW - 14, ey, 0.76f, zero ? -7702689 : -9807288);
        }
    ...
    private void renderRecentTasks(GuiGraphics gfx, int x, int y, int w, int h) {
        if (this.tasks.isEmpty()) {
            this.drawScaledString(gfx, SectScreen.tr("no_tasks"), x + 4, y + 4, 0.78f, -9807288, w - 8);
            return;
        }
        int rowH = 20;
        ...
        for (int i = 0; i < Math.min(max, this.tasks.size()); ++i) {
            ...
            this.drawScaledString(gfx, Component.translatable(task.titleKey), x + 8, ry + 4, 0.78f, -11911632, w - 60);
    ...
    private void renderMembersTab(GuiGraphics gfx, int y, int h, int mx, int my) {
        ...
        this.drawDiamondHeaderTag(gfx, lx + 6, y + 4, listW - 12, SectScreen.tr("members"), SectScreen.tr("book.roster_count", this.members.size()), false);
    ...
    private void renderMemberList(GuiGraphics gfx, int x, int y, int w, int h, int mx, int my) {
        ...
        for (int i = this.memberScroll; i < end; ++i) {
            ...
            this.drawScaledString(gfx, Component.literal(row.name).withStyle(ChatFormatting.BOLD), x + 22, ry + 4, 0.82f, row.player ? -15067628 : -11911632, 116);
            ...
            this.contentHoverTips.add(new HoverTipRect(x, ry, rowW, rowH - 2, List.of(Component.literal(row.name).withStyle(ChatFormatting.BOLD), SectScreen.tr("member_meta", row.role.displayName(), row.contribution), SectScreen.tr(row.player ? "member_kind.player" : "member_kind.npc"))));
        }
    ...
    private void renderMemberDetail(GuiGraphics gfx, int x, int y, int w, int h) {
        ...
        if (row == null) {
            this.drawScaledCentered(gfx, SectScreen.tr("member_detail.empty"), x + w / 2, y + h / 2 - 6, 0.82f, -9807288, w - 20);
            return;
        }
        int sealCx = x + 34;
        int sealCy = y + 30;
        this.sealCircle(gfx, sealCx, sealCy, 22, SectScreen.roleColor(row.role), Component.literal(SectScreen.firstGlyph(row.name)));
        this.drawRoleBadge(gfx, sealCx - 5, sealCy + 6, 10, row.role, false);
        this.drawScaledString(gfx, Component.literal(row.name).withStyle(ChatFormatting.BOLD), x + 64, y + 12, 1.3f, -14081252, w - 70);
        this.drawTagPill(gfx, x + 64, y + 32, row.role.displayName(), -7705298);
        int cy = y + 60;
        this.drawDiamondHeader(gfx, x + 10, cy, w - 20, SectScreen.tr("book.member_data"));
        this.drawStatGridRow(gfx, x + 10, cy += 16, w - 20, SectScreen.tr("summary.contribution"), Component.literal(String.valueOf(row.contribution)), -4885446, SectScreen.tr("book.identity"), SectScreen.tr(row.player ? "book.real" : "book.npc"), row.player ? -13668780 : -11911632);
        this.drawStatGridRow(gfx, x + 10, cy += 14, w - 20, SectScreen.tr("book.role_label"), row.role.displayName(), -11911632, SectScreen.tr("book.rank"), Component.literal("#" + this.memberRank(row)), -11911632);
        this.drawDiamondHeader(gfx, x + 10, cy += 20, w - 20, SectScreen.tr("book.role_duty"));
        this.drawWrappedLimited(gfx, SectScreen.tr("book.role.desc." + row.role.name().toLowerCase()), x + 10, cy += 15, w - 20, 0.74f, -11911632, 3);
        this.drawDiamondHeader(gfx, x + 10, cy += 36, w - 20, SectScreen.tr("book.share"));
    ...
    private void renderRelationsTab(GuiGraphics gfx, int y, int h, int mx, int my) {
        ...
        this.drawDiamondHeaderTag(gfx, x + 8, y + 4, w - 16, SectScreen.tr("book.relations_title"), SectScreen.tr("book.count_members", this.members.size()), false);
    ...
    private void renderOrgChart(GuiGraphics gfx, int x, int y, int w, int h, int mx, int my) {
        ...
        for (MemberRow row : this.members) {
            byRole.computeIfAbsent(row.role, k -> new ArrayList<MemberRow>()).add(row);
        }
        SectRole[] roles = SectScreen.displayRoles();
        ...
        try {
            for (SectRole role : roles) {
                List<MemberRow> rows = byRole.getOrDefault(role, List.of());
    ...
    private void drawOrgNode(GuiGraphics gfx, int x, int y, int w, MemberRow row, int mx, int my, int regionTop, int regionBottom) {
        ...
        this.drawScaledString(gfx, Component.literal(row.name), x + 18, y + 3, 0.75f, row.player ? -15067628 : -11911632, w - 24);
        if (row.player) {
        ...
        this.contentHoverTips.add(new HoverTipRect(x, rectTop, w, rectBot - rectTop, List.of(Component.literal(row.name).withStyle(ChatFormatting.BOLD), SectScreen.tr("member_meta", row.role.displayName(), row.contribution), SectScreen.tr(row.player ? "member_kind.player" : "member_kind.npc"))));
    }
    ...
    private void renderTasksTab(GuiGraphics gfx, int y, int h, int mx, int my) {
        ...
        int dx = lx + listW + gap;
        this.drawCard(gfx, lx, y, listW, h);
        this.drawDiamondHeaderTag(gfx, lx + 6, y + 4, listW - 12, SectScreen.tr("book.task_board"), SectScreen.tr("book.task_board_count", this.readyTaskCount(), this.activeTaskCount()), false);
    ...
    private void renderTaskList(GuiGraphics gfx, int x, int y, int w, int h, int mx, int my) {
        if (this.tasks.isEmpty()) {
            this.drawScaledString(gfx, SectScreen.tr("no_tasks"), x + 4, y + 6, 0.8f, -9807288, w - 8);
            return;
        }
        int rowH = 46;
        int step = rowH + 2;
        ...
        for (int i = this.taskScroll; i < end; ++i) {
            ...
            this.drawScaledString(gfx, Component.translatable(task.titleKey).withStyle(ChatFormatting.BOLD), x + 9, ry + 4, 0.85f, -15067628, rowW - 76);
            ...
            if (!task.requiredStack.isEmpty()) {
                ...
                this.drawScaledString(gfx, Component.literal("×" + task.requiredStack.getCount()), cursor + 17, ly + 1, 0.72f, -11911632, 26);
                cursor += 44;
            }
            this.drawScaledString(gfx, Component.literal("→"), cursor, ly + 1, 0.78f, -9807288, 12);
    ...
    private void renderTaskDetail(GuiGraphics gfx, int x, int y, int w, int h) {
        ...
        if (task == null) {
            this.drawScaledCentered(gfx, SectScreen.tr("no_tasks"), x + w / 2, y + h / 2 - 6, 0.8f, -9807288, w - 20);
            return;
        }
        this.drawDiamondHeader(gfx, x + 8, y + 4, w - 16, Component.translatable(task.titleKey));
        int cy = y + 22;
        cy = this.drawWrappedLimited(gfx, Component.translatable(task.conditionKey), x + 10, cy, w - 20, 0.75f, -11911632, 3);
        this.drawStatGridRow(gfx, x + 10, cy += 6, w - 20, SectScreen.tr("issuer_label"), Component.literal(task.issuerName), -11911632, SectScreen.tr("book.role_label"), task.issuerRole.displayName(), -11911632);
        Component need = task.requiredCount > 0 ? Component.literal(Math.min(task.heldRequired, task.requiredCount) + "/" + task.requiredCount) : SectScreen.tr("book.dash");
        this.drawStatGridRow(gfx, x + 10, cy += 14, w - 20, SectScreen.tr("book.required"), need, task.ready ? -13668780 : -5750484, SectScreen.tr("book.state"), this.taskPhase(task), this.taskPhaseColor(task));
        this.drawDiamondHeader(gfx, x + 10, cy += 20, w - 20, SectScreen.tr("book.items"));
        int slotX = x + 24;
        this.drawDetailSlot(gfx, slotX, cy += 16, 26, task.requiredStack);
        this.drawScaledString(gfx, Component.literal("→"), slotX + 32, cy + 9, 1.0f, -9807288, 14);
        this.drawDetailSlot(gfx, slotX + 52, cy, 26, task.rewardStack);
        this.drawScaledString(gfx, SectScreen.tr("book.required"), slotX, cy + 28, 0.66f, -9807288, 26);
        this.drawScaledString(gfx, SectScreen.tr("book.reward"), slotX + 52, cy + 28, 0.66f, -9807288, 26);
        cy += 50;
        if (task.requiredCount > 0) {
            this.drawDiamondHeader(gfx, x + 10, cy, w - 20, SectScreen.tr("book.turn_progress"));
            int bw = w - 20;
            this.drawProgressBar(gfx, x + 10, cy += 16, bw, task.heldRequired, task.requiredCount, task.ready);
            this.drawScaledRight(gfx, task.ready ? SectScreen.tr("book.turn_done") : Component.literal(Math.min(task.heldRequired, task.requiredCount) + " / " + task.requiredCount), x + 10 + bw, cy + 9, 0.72f, task.ready ? -13668780 : -9807288);
            cy += 22;
        }
    ...
    private void bottomActionRow(GuiGraphics gfx, int mx, int my, boolean hasSect) {
        ...
        if (hasSect && task != null && this.targetEntityId >= 0 && !this.viewerEnemy && (task.canTurnIn || task.canAccept)) {
            if (task.canTurnIn) {
                ...
                label = SectScreen.tr("task.turn_in");
                accent = -14722744;
            } else {
                kind = ActionKind.TASK_ACCEPT;
                label = SectScreen.tr("task.accept");
                accent = -7723482;
            }
            taskId = task.id;
        } else if (hasSect && task != null && task.requiresIssuer && !this.viewerEnemy && !task.completed) {
            kind = ActionKind.TASK_TRACK;
            label = SectScreen.tr("task.track_issuer");
            accent = -4818904;
            taskId = task.id;
        } else if (this.canJoin && this.targetEntityId >= 0) {
            kind = ActionKind.JOIN;
            label = SectScreen.tr("book.action.join");
            accent = -14722744;
        } else if (this.viewerMember) {
            kind = ActionKind.IMMUNITY;
            label = this.sameSectImmunity ? SectScreen.tr("same_immunity_on") : SectScreen.tr("same_immunity_off");
            accent = this.sameSectImmunity ? -14722744 : -9807288;
        } else if (this.viewerEnemy) {
            kind = ActionKind.NONE;
            label = SectScreen.tr("book.action.locked");
            accent = -7723482;
        }
        Component closeLabel = SectScreen.tr("book.action.close");
    ...
    private void renderBookSpine(GuiGraphics gfx, boolean hasSect) {
        ...
        String text = hasSect ? SectScreen.trimChars(this.snapshot.getString("sectName"), 4) : SectScreen.tr("side_label").getString();
    ...
    private void drawBrushTitle(GuiGraphics gfx, int centerX, int y, Component titleLine) {
        ...
        gfx.drawCenteredString(this.font, this.trimToWidth(bold, textW - 18), centerX, y + 8, -528679);
    }
    ...
    private Component tabLabel(Tab value) {
        return SectScreen.tr("tab." + SectScreen.tabKey(value));
    }
    ...
    private void drawNoSectState(GuiGraphics gfx) {
        ...
        this.drawScaledCentered(gfx, SectScreen.tr("seal"), x + w / 2, y + h / 2 - 36, 2.0f, -7702689, 60);
        this.drawScaledCentered(gfx, SectScreen.tr("none_title").copy().withStyle(ChatFormatting.BOLD), x + w / 2, y + h / 2 + 6, 1.0f, -14081252, w - 40);
        this.drawScaledCentered(gfx, SectScreen.tr("none_body"), x + w / 2, y + h / 2 + 22, 0.8f, -9807288, w - 60);
    }
    ...
    private void drawDiamondHeader(GuiGraphics gfx, int x, int y, int w, Component label) {
        ...
        gfx.drawCenteredString(this.font, this.trimToWidth(bold, w - 44), cx, y, -14081252);
    }
    ...
    private void drawStatCell(GuiGraphics gfx, int x, int y, int w, Component label, Component value, int valueColor) {
        ...
        this.drawScaledString(gfx, value.copy().withStyle(ChatFormatting.BOLD), x + 42, y, 1.0f, valueColor, Math.max(10, w - 42));
    }
    ...
    private int[] drawActionButton(GuiGraphics gfx, int x, int y, int w, int h, Component label, int accent, boolean enabled, int mx, int my) {
        ...
        Component component = this.trimToWidth(label.copy().withStyle(ChatFormatting.BOLD), w - 8);
        int n2 = x + w / 2;
        Objects.requireNonNull(this.font);
    ...
    private void sealCircle(GuiGraphics gfx, int cx, int cy, int r, int ring, Component glyph) {
        ...
        this.drawScaledCentered(gfx, glyph.copy().withStyle(ChatFormatting.BOLD), cx, cy - 8, 1.6f, ring, r * 2 - 6);
    }
    ...
    private int drawWrappedLimited(GuiGraphics gfx, Component text, int x, int y, int maxWidth, float scale, int color, int maxLines) {
        List<FormattedCharSequence> lines = this.font.split((FormattedText) text, (int) ((float) maxWidth / scale));
    ...
    private Component trimToWidth(Component text, int maxWidth) {
        ...
        return Component.literal(this.font.plainSubstrByWidth(value, Math.max(0, maxWidth - this.font.width("..."))) + "...").withStyle(text.getStyle());
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (SectScreen.inRect(this.closeButtonRect, lx, ly = this.lastViewport.toLocalY((int) mouseY))) {
    ...
    private void handlePrimaryAction() {
        switch (this.actionPrimaryKind) {
            case JOIN -> {
                ModNetwork.CHANNEL.sendToServer(new RequestSectJoinDialoguePacket(this.targetEntityId));
                Minecraft.getInstance().setScreen(null);
            }
            case IMMUNITY -> this.toggleImmunity();
            case TASK_TURN_IN -> {
                if (this.targetEntityId >= 0 && this.actionPrimaryTaskId != null) {
                    ModNetwork.CHANNEL.sendToServer(new SectTaskActionPacket(this.targetEntityId, this.actionPrimaryTaskId, true));
                }
            }
            case TASK_ACCEPT -> {
                if (this.targetEntityId >= 0 && this.actionPrimaryTaskId != null) {
                    ModNetwork.CHANNEL.sendToServer(new SectTaskActionPacket(this.targetEntityId, this.actionPrimaryTaskId, false));
                }
            }
            case TASK_TRACK -> {
                if (this.actionPrimaryTaskId != null) {
                    ModNetwork.CHANNEL.sendToServer(new TrackSectTaskIssuerPacket(this.actionPrimaryTaskId));
                }
            }
        }
    }
    ...
    private Component infoNotice() {
        if (this.viewerEnemy) {
            return SectScreen.tr("book.notice.hostile");
        }
        if (this.viewerMember) {
            return SectScreen.tr(this.sameSectImmunity ? "book.notice.member" : "book.notice.member_off");
        }
        return SectScreen.tr("book.notice.guest");
    }

    private Component relationStatus() {
        if (this.viewerEnemy) {
            return SectScreen.tr("book.relation.hostile");
        }
        if (this.viewerMember) {
            return SectScreen.tr("book.relation.member");
        }
        return SectScreen.tr("book.relation.guest");
    }
    ...
    private Component taskStatus(TaskRow task) {
        if (task.completed) {
            return SectScreen.tr("task.completed");
        }
        if (this.viewerEnemy) {
            return SectScreen.tr("task.hostile_locked");
        }
        if (!this.viewerMember) {
            return SectScreen.tr("task.join_first");
        }
        if (!task.accepted) {
            return task.requiresIssuer ? SectScreen.tr("task.requires_issuer") : SectScreen.tr("task.can_accept");
        }
        if (task.ready) {
            if (!task.rewardAvailable && !task.requiresIssuer) {
                return SectScreen.tr("task.reward_unavailable");
            }
            return task.requiresIssuer ? SectScreen.tr("task.ready_find_issuer") : SectScreen.tr("task.ready");
        }
        return SectScreen.tr("task.accepted");
    }

    private Component taskPhase(TaskRow task) {
        String b = "task.phase.";
        if (task.completed) {
            return SectScreen.tr(b + "completed");
        }
        if (this.viewerEnemy) {
            return SectScreen.tr(b + "hostile");
        }
        if (!this.viewerMember) {
            return SectScreen.tr(b + "join_first");
        }
        if (!task.accepted) {
            return SectScreen.tr(b + "accept");
        }
        if (task.ready) {
            return SectScreen.tr(b + (task.rewardAvailable ? "report" : "blocked"));
        }
        return SectScreen.tr(b + "collect");
    }
    ...
    private Component taskNextStep(TaskRow task) {
        ...
        MutableComponent issuer = Component.literal(task.issuerName);
        if (!task.accepted) {
        ...
        return SectScreen.tr(b + "accepted");
    }
