...
/**
 * 宗门加入对话框（完整复刻原模组 com.xiaoxiang.cultivation.client.screen.SectJoinDialogueScreen，1152 行）。
 */
...
extends Screen {
    ...
    private static final ResourceLocation CHOICE_BRUSH_TEXTURE = new ResourceLocation("friday_cultivation", "textures/gui/sect_dialogue_choice_brush.png");
    private static final ResourceLocation MESSAGE_BRUSH_TEXTURE = new ResourceLocation("friday_cultivation", "textures/gui/sect_dialogue_message_brush.png");
    ...
    public SectJoinDialogueScreen(int targetEntityId, String sectName, String npcName, CompoundTag snapshot) {
        super(Component.translatable("screen.friday_cultivation.sect.dialogue.title", Component.literal(SectJoinDialogueScreen.safe(sectName))));
    ...
    private List<DialogueOption> dialogueOptions() {
        ...
        if (this.canJoin) {
            options.add(new DialogueOption(Component.translatable("screen.friday_cultivation.sect.dialogue.option.join"), () -> {
                ModNetwork.CHANNEL.sendToServer(new JoinSectPacket(this.targetEntityId));
                Minecraft.getInstance().setScreen(null);
            }, true, "join_option"));
        }
        for (TaskAction task : this.taskActions) {
            MutableComponent taskTitle = Component.translatable(task.titleKey());
            String key = task.turnIn() ? ".sect.dialogue.task.turn_in_option" : ".sect.dialogue.task.accept_option";
            options.add(new DialogueOption(Component.translatable("screen.friday_cultivation" + key, taskTitle), () -> {
                ModNetwork.CHANNEL.sendToServer(new SectTaskActionPacket(this.targetEntityId, task.id(), task.turnIn()));
                Minecraft.getInstance().setScreen(null);
            }, true, task.turnIn() ? "task_turn_in_option" : "task_accept_option"));
        }
        options.add(new DialogueOption(Component.translatable("screen.friday_cultivation.sect.dialogue.option.leave"), () -> Minecraft.getInstance().setScreen(null), false, "leave_option"));
        return options;
    }
    ...
    private void renderNpcFullBody(GuiGraphics gfx, LivingEntity npc) {
        ...
        renderEntityInInventory(gfx, x, baseY, scale, -58.0f, -18.0f, npc);
    }
    ...
    private void drawBrushChoice(GuiGraphics gfx, int x, int y, int width, int height, Component label, boolean hovered, boolean emphasis) {
        ...
        Component component = this.trimToWidth(label.plainCopy().withStyle(ChatFormatting.BOLD), textMax);
        int n = x + width / 2;
        float f = height;
    ...
    private void renderNpcPortrait(GuiGraphics gfx, LivingEntity npc, int x, int y, int size) {
        ...
        renderEntityInInventory(gfx, x + size / 2, y + size - 4, scale, -20.0f, -10.0f, npc);
    }
    ...
    private static int brushNib(int index, int width, boolean rightSide) {
        int[] pattern;
        if (rightSide) {
            int[] right = new int[]{3, 10, 1, 15, 6, 20, 2, 12, 8, 0, 17};
            pattern = right;
        } else {
            int[] left = new int[]{14, 5, 0, 18, 8, 2, 21, 7, 12, 1, 16};
            pattern = left;
        }
    ...
    private int drawWrappedScaledLimited(GuiGraphics gfx, Component text, int x, int y, int maxWidth, int color, int maxLines, float scale) {
        ...
        for (FormattedCharSequence line : this.font.split(text, splitWidth)) {
    ...
    private void drawScaledCentered(GuiGraphics gfx, Component text, int centerX, int y, int color, float scale) {
        int drawX = Math.round((float) centerX - (float) this.font.width(text) * scale / 2.0f);
    ...
    private void renderPanel(GuiGraphics gfx, Layout layout, List<DialogueOption> options, int mouseX, int mouseY) {
        ...
        MutableComponent header = Component.translatable("screen.friday_cultivation.sect.dialogue.header", Component.literal(this.npcName), Component.literal(this.sectName));
        this.drawSectSeal(gfx, layout.left + 14, layout.top + 12, 14, this.headerAccentColor());
        gfx.drawString(this.font, this.trimToWidth(header.plainCopy().withStyle(ChatFormatting.BOLD), layout.width - 54), layout.left + 34, layout.top + 9, -7723482, false);
        ...
        if (besideButtons) {
            ...
            gfx.drawString(this.font, Component.translatable("screen.friday_cultivation" + (this.viewerEnemy ? ".sect.dialogue.action_hint_hostile" : ".sect.dialogue.action_hint")), optionX, layout.top + 43, this.viewerEnemy ? -7723482 : -7702689, false);
    ...
    private void drawHeaderMeta(GuiGraphics gfx, int x, int y, int width) {
        ...
        MutableComponent npcRole = Component.translatable("screen.friday_cultivation.sect.dialogue.npc_role", this.targetRole.displayName());
        if (this.viewerEnemy) {
            viewer = Component.translatable("screen.friday_cultivation.sect.dialogue.viewer_hostile");
            viewerColor = -7723482;
        } else if (this.viewerMember) {
            viewer = Component.translatable("screen.friday_cultivation.sect.dialogue.viewer_role", this.viewerRole.displayName(), this.viewerContribution);
            viewerColor = SectJoinDialogueScreen.roleColor(this.viewerRole);
        } else {
            viewer = Component.translatable("screen.friday_cultivation.sect.dialogue.viewer_guest");
            viewerColor = -7700107;
        }
        int roleW = Math.min(width / 2, Math.max(86, this.font.width(npcRole) + 12));
        this.drawChip(gfx, x, y, roleW, npcRole, SectJoinDialogueScreen.roleColor(this.targetRole));
        this.drawChip(gfx, x + roleW + 6, y, width - roleW - 6, viewer, viewerColor);
    }
    ...
    private void drawContent(GuiGraphics gfx, int x, int y, int width, int height) {
        ...
        if (this.viewerEnemy) {
            int noticeY = cursorY += 5;
            int noticeH = this.drawNoticeCard(gfx, x + 7, noticeY, width - 14, Component.translatable("screen.friday_cultivation.sect.dialogue.hostile_notice"), -7723482);
            ...
            int noticeY = cursorY += 5;
            int noticeH = this.drawNoticeCard(gfx, x + 7, noticeY, width - 14, Component.translatable("screen.friday_cultivation.sect.dialogue.join_outcome"), -14722744);
        ...
        if (!this.taskLines.isEmpty() && cursorY + 22 < y + height) {
            ...
            MutableComponent heading = this.taskLines.size() > visibleCards ? Component.translatable("screen.friday_cultivation.sect.dialogue.task_overview_count", this.taskScroll + 1, this.taskScroll + visibleCards, this.taskLines.size()) : Component.translatable("screen.friday_cultivation.sect.dialogue.task_overview");
            gfx.drawString(this.font, heading.plainCopy().withStyle(ChatFormatting.BOLD), x + 8, cursorY + 2, -7723482, false);
            if (this.taskLines.size() > visibleCards) {
                MutableComponent hint = Component.translatable("screen.friday_cultivation.sect.dialogue.task_scroll_hint");
                gfx.drawString(this.font, this.trimToWidth(hint, width - 132), x + width - 126, cursorY + 2, -7702689, false);
            }
    ...
    private int drawNoticeCard(GuiGraphics gfx, int x, int y, int width, Component text, int accent) {
        List<FormattedCharSequence> lines = this.font.split(text, width - 30);
    ...
    private void drawTaskCard(GuiGraphics gfx, TaskLine task, int x, int y, int width) {
        ...
        gfx.drawString(this.font, this.trimToWidth(Component.translatable(task.titleKey()).plainCopy().withStyle(ChatFormatting.BOLD), showPhase ? phaseX - x - 28 : statusX - x - 28), x + 22, y + 5, -15067628, false);
        if (showPhase) {
        ...
        MutableComponent requirement = task.requiredStack().isEmpty() ? Component.translatable(task.conditionKey()) : Component.translatable("screen.friday_cultivation.sect.require_line", task.requiredStack().getHoverName(), task.requiredStack().getCount());
        MutableComponent progress = task.requiredCount() > 0 ? Component.translatable("screen.friday_cultivation.sect.progress_line", Math.min(task.heldRequired(), task.requiredCount()), task.requiredCount()) : null;
        int infoY = y + 24;
        int infoH = 38;
        int infoX = x + 7;
        int infoW = width - 14;
        ...
        this.drawTaskInfoBox(gfx, infoX, infoY, requiredW, infoH, -4818904, task.requiredStack(), requirement, progress, -12766422, task.ready() ? -14722744 : -9807288, task.ready());
        ...
        MutableComponent reward = task.rewardStack().isEmpty() ? Component.translatable("screen.friday_cultivation.sect.reward_contribution_only", task.contribution()) : Component.translatable("screen.friday_cultivation.sect.reward_line", task.rewardStack().getHoverName(), task.contribution());
        Component source = this.rewardSourceLabel(task.rewardKind());
        this.drawTaskInfoBox(gfx, rewardX, infoY, rewardW, infoH, SectJoinDialogueScreen.rewardSourceColor(task.rewardKind()), task.rewardStack(), reward, source, task.rewardAvailable() ? -7723482 : -7700107, SectJoinDialogueScreen.rewardSourceColor(task.rewardKind()), task.rewardAvailable());
    ...
    private static HoverTipRect tooltipRect(int x, int y, int width, int height, String suffix) {
        return new HoverTipRect(x, y, width, height, List.of(Component.translatable("screen.friday_cultivation.sect.dialogue.tooltip." + suffix)));
    }

    private static HoverTipRect sectTooltipRect(int x, int y, int width, int height, String suffix) {
        return new HoverTipRect(x, y, width, height, List.of(Component.translatable("screen.friday_cultivation.sect.tooltip." + suffix)));
    }

    private Component bodyText() {
        if (this.viewerEnemy) {
            return Component.translatable("screen.friday_cultivation.sect.dialogue.hostile_greeting", Component.literal(this.npcName), Component.literal(this.sectName));
        }
        if (this.canJoin) {
            return Component.translatable("screen.friday_cultivation.sect.dialogue.master_greeting", Component.literal(this.npcName), Component.literal(this.sectName));
        }
        if (!this.taskActions.isEmpty()) {
            return Component.translatable("screen.friday_cultivation.sect.dialogue.task_greeting", Component.literal(this.npcName));
        }
        if (this.viewerMember) {
            return Component.translatable("screen.friday_cultivation.sect.dialogue.no_task_member", Component.literal(this.npcName));
        }
        return Component.translatable("screen.friday_cultivation.sect.dialogue.no_task_guest", Component.literal(this.npcName));
    }

    private Component actionSummary() {
        String base = "screen.friday_cultivation.sect.dialogue.reply_summary.";
        if (this.viewerEnemy) {
            return Component.translatable(base + "hostile");
        }
        if (this.canJoin) {
            return Component.translatable(base + "join");
        }
        if (!this.taskActions.isEmpty()) {
            return Component.translatable(base + "tasks", this.taskActions.size());
        }
        return Component.translatable(base + (this.viewerMember ? "no_actions_member" : "no_actions_guest"));
    }
    ...
    private void drawDialogueButton(GuiGraphics gfx, int x, int y, int width, Component label, boolean hovered, boolean emphasis) {
        int border = emphasis ? -4703686 : -3562934;
        int n = hovered ? -7723482 : border;
        int fill = hovered ? -7770 : (emphasis ? -995445 : -1586805);
        gfx.fill(x, y, x + width, y + 20, n);
        gfx.fill(x + 1, y + 1, x + width - 1, y + 20 - 1, fill);
        gfx.fill(x + 1, y + 1, x + 4, y + 20 - 1, n);
        if (hovered) {
            gfx.fill(x + 4, y + 1, x + width - 1, y + 3, 0x55FFFFFF);
        }
        this.drawMiniGlyph(gfx, x + 7, y + 6, 8, n, emphasis);
    ...
    private Component taskStatus(TaskLine task) {
        if (this.viewerEnemy) {
            return Component.translatable("screen.friday_cultivation.sect.task.hostile_locked");
        }
        if (task.canTurnIn()) {
            return Component.translatable("screen.friday_cultivation.sect.dialogue.task_status_turn_in");
        }
        if (task.canAccept()) {
            return Component.translatable("screen.friday_cultivation.sect.dialogue.task_status_accept");
        }
        if (task.ready() && !task.rewardAvailable()) {
            return Component.translatable("screen.friday_cultivation.sect.task.reward_unavailable");
        }
        if (task.ready()) {
            return Component.translatable("screen.friday_cultivation.sect.task.ready_find_issuer");
        }
        if (task.accepted()) {
            return Component.translatable("screen.friday_cultivation.sect.task.accepted");
        }
        return Component.translatable("screen.friday_cultivation.sect.dialogue.task_status_locked");
    }

    private Component taskPhase(TaskLine task) {
        String base = "screen.friday_cultivation.sect.task.phase.";
        if (task.completed()) {
            return Component.translatable(base + "completed");
        }
        if (this.viewerEnemy) {
            return Component.translatable(base + "hostile");
        }
        if (!this.viewerMember) {
            return Component.translatable(base + "join_first");
        }
        if (!task.accepted()) {
            return Component.translatable(base + "accept");
        }
        if (task.ready()) {
            if (!task.rewardAvailable()) {
                return Component.translatable(base + "blocked");
            }
            return Component.translatable(base + "report");
        }
        return Component.translatable(base + "collect");
    }
    ...
    private Component taskNextStep(TaskLine task) {
        ...
        if (!task.accepted()) {
            if (task.canAccept()) {
                return Component.translatable(base + "accept_here", issuer);
            }
            return Component.translatable(base + "accept_issuer", issuer);
        }
        if (task.ready()) {
            if (!task.rewardAvailable()) {
                return Component.translatable(base + "reward_unavailable", issuer);
            }
            if (task.canTurnIn()) {
                return Component.translatable(base + "turn_in_here", issuer);
            }
            return Component.translatable(base + "turn_in_issuer", issuer);
        }
        if (task.requiredCount() > 0) {
            return Component.translatable(base + "collect", Math.min(task.heldRequired(), task.requiredCount()), task.requiredCount());
        }
        return Component.translatable(base + "accepted");
    }

    private Component rewardSourceLabel(String rewardKind) {
        return Component.translatable("screen.friday_cultivation.sect.reward_source." + SectJoinDialogueScreen.rewardSourceSuffix(rewardKind));
    }
    ...
    private Component trimToWidth(Component label, int maxWidth) {
        ...
        String ellipsis = "...";
        return Component.literal(this.font.plainSubstrByWidth(label.getString(), Math.max(0, maxWidth - this.font.width(ellipsis))) + ellipsis);
    }
    ...
    /**
     * 渲染实体到背包栏位置（照搬原模组 InventoryScreen.renderEntityInInventory float 版）。
     * 1.20.1 Forge 编译时找不到 float 重载（SRG 映射问题），用反射调用运行时存在的方法，
     * 与 CultivationScreen.renderPlayerModel 同一方案。
     */
    private static void renderEntityInInventory(GuiGraphics gfx, int x, int y, int scale, float mouseX, float mouseY, LivingEntity entity) {
        if (entity == null) {
            return;
        }
        try {
            java.lang.reflect.Method m = InventoryScreen.class.getMethod("m_274545_", GuiGraphics.class, int.class, int.class, int.class, float.class, float.class, LivingEntity.class);
            m.invoke(null, gfx, x, y, scale, mouseX, mouseY, entity);
        } catch (NoSuchMethodException e1) {
            try {
                java.lang.reflect.Method m = InventoryScreen.class.getDeclaredMethod("renderEntityInInventory", GuiGraphics.class, int.class, int.class, int.class, float.class, float.class, LivingEntity.class);
                m.setAccessible(true);
                m.invoke(null, gfx, x, y, scale, mouseX, mouseY, entity);
            } catch (Exception e2) {
                renderEntityInInventoryFallback(gfx, x, y, scale, mouseX, mouseY, entity);
            }
        } catch (Exception e) {
            renderEntityInInventoryFallback(gfx, x, y, scale, mouseX, mouseY, entity);
        }
    ...
    /** 手动渲染 fallback（参考 vanilla InventoryScreen.renderEntityInInventory 源码） */
    private static void renderEntityInInventoryFallback(GuiGraphics gfx, int x, int y, int scale, float mouseX, float mouseY, LivingEntity entity) {
        net.minecraft.client.Minecraft mc = net.minecraft.client.Minecraft.getInstance();
        com.mojang.blaze3d.vertex.PoseStack pose = gfx.pose();
        pose.pushPose();
        pose.translate(x, y, 50.0);
        pose.scale(scale, -scale, scale);
        org.joml.Quaternionf rotation = new org.joml.Quaternionf().rotationYXZ((float) Math.toRadians(180.0), 0.0f, (float) Math.toRadians(180.0));
        org.joml.Quaternionf mouseRot = new org.joml.Quaternionf().rotationYXZ(mouseY * 0.02f, mouseX * 0.02f, 0.0f);
