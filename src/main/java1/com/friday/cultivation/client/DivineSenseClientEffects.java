...
public final class DivineSenseClientEffects {
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        long now = System.currentTimeMillis();
        REVEALED_BLOCKS.entrySet().removeIf(entry -> now >= entry.getValue());
        ...
        while (scanIt.hasNext()) {
            ...
            while (blockIt.hasNext()) {
                ...
                if (!(Vec3.atLowerCornerOf(pos).distanceTo(scan.center) <= radius)) continue;
                REVEALED_BLOCKS.put(pos.immutable(), revealUntil);
                blockIt.remove();
            }
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        if (!ACTIVE_SCANS.isEmpty()) {
            ...
            for (ActiveScan scan : ACTIVE_SCANS) {
                ...
                float fade = now <= scan.endMs ? 1.0f : 1.0f - Mth.clamp((float)(now - scan.endMs) / 600.0f, 0.0f, 1.0f);
    ...
    private static void renderCountdown(GuiGraphics gfx, int screenWidth, int screenHeight) {
        ...
        long totalMs = 30000L;
        float progress = Mth.clamp((float)remainMs / (float)totalMs, 0.0f, 1.0f);
        ...
        int barY = 21;
        Font font = mc.font;
        MutableComponent label = Component.translatable("hud.friday_cultivation.divine_sense.countdown", String.valueOf(remainSec));
        int textColor = -2569985;
        int labelW = font.width(label);
        gfx.drawString(font, label, (screenWidth - labelW) / 2, 10, textColor, true);
        int INK = -15067628;
        int TRACK = -869653472;
        int FILL = -6125344;
    ...
    private static final class ActiveScan {
        ...
        private double progress(long now) {
            return Mth.clamp((double)(now - this.startMs) / (double)Math.max(1L, this.endMs - this.startMs), 0.0, 1.0);
        }
