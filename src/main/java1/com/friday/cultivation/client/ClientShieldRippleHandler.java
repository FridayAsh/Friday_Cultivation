...
public final class ClientShieldRippleHandler {
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        while (it.hasNext()) {
            ...
            BlockPos center = BlockPos.of(e.getKey());
            for (int dx = -2; dx <= 2; ++dx) {
                for (int dy = -2; dy <= 2; ++dy) {
                    for (int dz = -2; dz <= 2; ++dz) {
                        ...
                        if (dist > 2 || !ClientShieldRippleHandler.shouldRenderRippleBlock(st, p)) {
                            continue;
                        }
                        float alpha = ClientShieldRippleHandler.baseAlphaForDistance(dist) * timeFade;
                        if (alpha < 0.01f) {
                            continue;
                        }
