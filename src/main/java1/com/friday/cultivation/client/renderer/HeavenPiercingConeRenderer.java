...
extends EntityRenderer<HeavenPiercingConeEntity> {
    private static final ResourceLocation WHITE = new ResourceLocation((String)"minecraft:textures/misc/white.png");
    ...
    public static void renderCone(PoseStack pose, MultiBufferSource buffers, Vec3 direction, int stage, int chargeTicks, float ageTicks, int packedLight, float alphaMultiplier) {
        ...
        pose.mulPose(Axis.ZN.rotationDegrees(ageTicks * (18.0f + (float)safeStage * 16.0f + (float)chargeTicks * 0.08f)));
        ...
        VertexConsumer body = buffers.getBuffer(RenderType.entitySolid((ResourceLocation)WHITE));
        HeavenPiercingConeRenderer.emitStoneModel(pose, body, length, radius, palette, alphaByte, packedLight);
        VertexConsumer glow = buffers.getBuffer(RenderType.entityCutoutNoCull((ResourceLocation)WHITE));
    ...
    private static void emitGlowModel(PoseStack pose, VertexConsumer vc, float length, float radius, ConePalette palette, int stage, int chargeTicks, int alpha) {
        ...
        float pulse = 0.82f + Mth.cos((float)((float)chargeTicks * 0.22f)) * 0.18f;
    ...
    private static float facetScale(int segment, int ring) {
        ...
        return 1.0f + uneven + Mth.cos((float)((float)segment * 1.37f + (float)ring * 0.71f)) * 0.035f;
    }
