...
extends EntityRenderer<SwordProjectileEntity> {
    ...
    private void renderStreak(SwordProjectileEntity entity, PoseStack pose, MultiBufferSource buf, float partialTick) {
        ArrayList history = new ArrayList(entity.getTrailHistory());
        if (history.size() < 2) {
            return;
        }
        Vec3 renderPos = new Vec3(Mth.lerp((double)partialTick, (double)entity.xo, (double)entity.getX()), Mth.lerp((double)partialTick, (double)entity.yo, (double)entity.getY()), Mth.lerp((double)partialTick, (double)entity.zo, (double)entity.getZ()));
        ...
        for (int i = 0; i < count - 1; ++i) {
            ...
            Vec3 lp0 = worldP0.subtract(renderPos);
            Vec3 lp1 = worldP1.subtract(renderPos);
    ...
    private static void addStreakVert(VertexConsumer vc, Matrix4f m, Matrix3f n, float x, float y, float z, float u, float v, int alpha) {
        vc.vertex(m, x, y, z).color(255, 255, 255, alpha).uv(u, v).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(0xF000F0).normal(n, 0.0f, 1.0f, 0.0f).endVertex();
    }
    @NotNull
