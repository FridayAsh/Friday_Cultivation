...
/**
 * 锁灵阵视觉处理器 - 被锁定的生物/方块周围绘制锁链。
 * 完全照搬原 mod: xiaoxiang.cultivation.client.SpiritLockVisualHandler
 */
...
public final class SpiritLockVisualHandler {
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        if (hasBlockLocks) {
            ...
            while (it.hasNext()) {
                ...
                SpiritLockChainModel.renderBlock(pose, buffers, age, SpiritLockVisualHandler.alpha(endMs));
                pose.popPose();
            }
        }
        if (firstPersonLockEndMs != null) {
            SpiritLockVisualHandler.renderFirstPersonLock(event.getCamera(), pose, buffers, age, SpiritLockVisualHandler.alpha(firstPersonLockEndMs));
        }
        pose.popPose();
    ...
    private static void renderFirstPersonLock(Camera camera, PoseStack pose, MultiBufferSource buffers, float age, float alpha) {
        ...
        Vec3 center = origin.add(forward.scale(0.78)).subtract(up.scale(0.03));
