...
public final class BuddhaFireLotusClientEffects {
    ...
    public static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        for (LotusExplosionEffect effect : ACTIVE) {
            ...
            if (shockProgress < 1.0f) {
                ...
                BuddhaFireLotusRenderer.renderCyanShockwave(pose, buffers, shockRadius, 0.28f + shockRadius * 0.018f, shockAlpha);
            }
            BuddhaFireLotusRenderer.renderLotusLightning(pose, buffers, (float)effect.radius, burstProgress, age, alpha);
            BuddhaFireLotusRenderer.renderLotus(pose, buffers, effect.rootFlags, effect.chargedQi, age, scale, alpha, age * 0.55f, bloom);
            pose.popPose();
            rendered = true;
        }
        if (rendered) {
    ...
    private static void spawnLotusQiParticles(Minecraft mc, LotusExplosionEffect effect) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            level.addParticle(particle, x, y, z, -Math.sin(angle) * swirl, lift, Math.cos(angle) * swirl);
        }
    }

    private static SimpleParticleType lotusQiParticle(int index) {
        return switch (Math.floorMod(index, 3)) {
            case 0 -> ModParticles.AMBIENT_QI_LOTUS.get();
            case 1 -> ModParticles.AMBIENT_QI_LIGHTNING.get();
            default -> ModParticles.AMBIENT_QI_WATER.get();
        };
    }
    ...
    private static void spawnLotusFireStreams(Minecraft mc, LotusExplosionEffect effect) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            Vec3 velocity = tangent.scale(swirl).add(outward.scale(spread)).add(0.0, -0.015 + level.random.nextDouble() * 0.095, 0.0);
            level.addParticle(cyan ? ParticleTypes.SOUL_FIRE_FLAME : ParticleTypes.CLOUD, x, y, z, velocity.x, velocity.y, velocity.z);
            level.addParticle(cyan ? LOTUS_CYAN_FIRE_DUST : LOTUS_WHITE_FIRE_DUST, x, y + 0.04, z, velocity.x * 0.62, velocity.y * 0.72, velocity.z * 0.62);
            if (cyan || (i & 3) != 0) continue;
            level.addParticle(ParticleTypes.END_ROD, x, y + 0.03, z, velocity.x * 0.34, velocity.y * 0.52, velocity.z * 0.34);
        }
    }
    ...
    private static void renderOverlay(GuiGraphics graphics, int screenWidth, int screenHeight) {
        ...
        for (LotusExplosionEffect effect : ACTIVE) {
            if (effect.remainingTicks <= 0) continue;
            double dist = player.position().distanceTo(effect.center);
            double influence = 1.0 - Math.min(1.0, dist / Math.max(1.0, effect.radius + 24.0));
