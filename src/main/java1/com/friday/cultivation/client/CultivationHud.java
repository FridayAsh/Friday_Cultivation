...
public final class CultivationHud {
    ...
    private static final ResourceLocation HUD_INK_BACKDROP = new ResourceLocation("friday_cultivation", (String)"textures/gui/hud_ink_backdrop.png");
    ...
    private static void render(GuiGraphics graphics, int screenWidth) {
        ...
        boolean soul = cap.isSoulState();
        if (!soul && !cap.hasEquippedTechnique()) {
            return;
        }
        int x = CultivationHud.hudX(screenWidth);
        int y = 6;
        Realm realm = cap.getRealm();
        MutableComponent realmLine = cap.isLooseImmortal() ? Component.translatable((String)"hud.friday_cultivation.realm", (Object[])new Object[]{Component.literal((String)("realm.friday_cultivation.loose_immortal.level." + cap.getLooseImmortalTribulations()))}) : (realm == Realm.MORTAL || realm == Realm.TRUE_IMMORTAL ? Component.translatable((String)"hud.friday_cultivation.realm", (Object[])new Object[]{realm.displayName()}) : Component.translatable((String)"hud.friday_cultivation.realm_full", (Object[])new Object[]{realm.displayName(), cap.getSubStage().displayName()}));
        long curQi = cap.getCurrentQi();
        long maxQi = cap.getMaxQi();
        long curCult = cap.getCultivationProgress();
        long maxCult = cap.getMaxCultivation();
        ...
        if (cap.hasActiveInverseFiveElementMark(gameTime = player.level().getGameTime())) {
            CultivationHud.drawInverseFiveElementStatus(graphics, mc, cap, gameTime, x + 8, statusY, 104);
            statusY += 10;
        }
        if (cap.isInTribulation()) {
            MutableComponent trib = Component.translatable((String)"hud.friday_cultivation.tribulation", (Object[])new Object[]{Realm.formatTribulationCount(cap.getTribulationStrikesRemaining(), cap.getTribulationBoltsPerWave())}).withStyle(ChatFormatting.RED);
            CultivationHud.drawPlainStatus(graphics, mc, (Component)trib, x + 8, statusY, 104, -35483, false);
            statusY += 9;
        }
        if (soul) {
            CultivationHud.renderSoulStatus(graphics, mc, player, cap, x, statusY);
        }
    }
    ...
    private static ResourceLocation spiritRootIcon(QiElement element) {
        ...
        return new ResourceLocation("friday_cultivation", (String)(SPIRIT_ROOT_ICON_PREFIX + id + ".png"));
    }
    ...
    public static void register(net.minecraftforge.client.event.RegisterGuiOverlaysEvent event) {
        event.registerAbove(net.minecraftforge.client.gui.overlay.VanillaGuiOverlay.HOTBAR.id(), "cultivation_hud", OVERLAY);
    }

    public static ResourceLocation overlayId() {
        return new ResourceLocation("friday_cultivation", (String)"cultivation_hud");
    }
}
