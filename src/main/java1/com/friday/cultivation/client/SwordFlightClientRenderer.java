...
public final class SwordFlightClientRenderer {
    ...
    public static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        CultivationData data = CultivationCapability.get(mc.player).orElse(null);
        ...
        float yaw = bodyAnchor.map(NascentSoulBodyVisualHandler.BodyAnchor::yRot).orElseGet(() -> Float.valueOf(Mth.rotLerp(partial, mc.player.yRotO, mc.player.getYRot()))).floatValue();
        ...
        mc.getItemRenderer().renderStatic(sword, ItemDisplayContext.FIXED, 0xF000F0, OverlayTexture.NO_OVERLAY, pose, buffer, mc.level, mc.player.getId());
        buffer.endBatch();
        pose.popPose();
    }

    private static Vec3 interpolatedPlayerPosition(Minecraft mc, float partial) {
        return new Vec3(Mth.lerp(partial, mc.player.xo, mc.player.getX()), Mth.lerp(partial, mc.player.yo, mc.player.getY()), Mth.lerp(partial, mc.player.zo, mc.player.getZ()));
    }
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        for (AbstractClientPlayer player : mc.level.players()) {
            if (!SwordFlightClientRenderer.isSwordFlightActive(player)) continue;
            SwordFlightClientRenderer.freezeWalkAnimation(player);
        }
        if (mc.player == null || !SwordFlightClientRenderer.isSwordFlightActive(mc.player)) {
            TRAIL_HISTORY.clear();
            return;
        }
    ...
    private static void renderSwordTrail(PoseStack pose, MultiBufferSource.BufferSource buffer, Vec3 cameraPos, TrailSample headNow) {
        ...
        for (int i = 0; i < interpCount; ++i) {
            lp[i] = interp[i].subtract(cameraPos).add(0.0, -0.1, 0.0);
        ...
        VertexConsumer vc = buffer.getBuffer(RenderType.entityTranslucent(WHITE));
