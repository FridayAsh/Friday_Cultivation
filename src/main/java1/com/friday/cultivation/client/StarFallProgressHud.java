...
/**
 * 星陨/蓄力法术进度 HUD — 完全照搬原模组 com.xiaoxiang.cultivation.client.StarFallProgressHud。
 * 显示各类蓄力法术的灵气/蓄力进度条。
 */
public final class StarFallProgressHud {
    ...
    private static void render(GuiGraphics gfx, int screenWidth, int screenHeight) {
        ...
        gfx.fill(barX - 1, barY - 1, barX + BAR_W + 1, barY + BAR_H + 1, INK_BLACK);
        gfx.fill(barX, barY, barX + BAR_W, barY + BAR_H, BG_TRACK);
        int filled = (int) ((float) BAR_W * progress);
        if (filled > 0) {
            gfx.fill(barX, barY, barX + filled, barY + BAR_H, CINNABAR);
            gfx.fill(barX, barY, barX + filled, barY + 1, GOLD_BRIGHT);
            gfx.fill(barX, barY + BAR_H - 1, barX + filled, barY + BAR_H, CINNABAR_DEEP);
            if (progress >= 0.9f) {
                ...
                gfx.fill(barX, barY, barX + filled, barY + BAR_H, pulseColor);
            }
        }
        MutableComponent label = Component.translatable(labelKey);
        gfx.drawString(mc.font, label, barX, barY - 11, TEXT_LIGHT, true);
        ...
        int pctColor = displayPct > 100 ? GOLD_BRIGHT : TEXT_LIGHT;
        String pctText = displayPct + "%";
        int pctW = mc.font.width(pctText);
        gfx.drawString(mc.font, pctText, barX + BAR_W - pctW, barY - 11, pctColor, true);
    }
}
