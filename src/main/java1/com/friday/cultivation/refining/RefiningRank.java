...
public enum RefiningRank {
    ...
    RefiningRank(String id, ChatFormatting color, int xpToNext, int[] probabilities) {
        this.id = id;
        this.color = color;
    ...
    public Component displayName() {
        return Component.translatable("refining_rank.friday_cultivation." + this.id);
    }
    ...
    /** 掷骰决定炼器产物品阶（照搬原模组：i==0 表示失败返回 null） */
    ...
    /** 根据炼器熟练度(xp)获取对应等级（项目兼容方法，供 CultivationData 使用） */
    public static RefiningRank fromXp(int xp) {
        RefiningRank rank = APPRENTICE;
        int threshold = 0;
        for (RefiningRank r : values()) {
