...
/**
 * 地府村寨特征 - 完全照搬原 mod: xiaoxiang.cultivation.worldgen.DifuVillageFeature（233 行完整版）。
 * 在生成点周围放置 6 座下界砖房屋/塔楼、连接道路、中央路灯，并生成 4-6 名游方修士。
 */
public class DifuVillageFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(@NotNull FeaturePlaceContext<NoneFeatureConfiguration> ctx) {
        ...
        for (int s = 0; s < spots.length; ++s) {
            ...
            int bx = cx + spots[s][0];
            int bz = cz + spots[s][1];
            int gy = level.getHeight(Heightmap.Types.WORLD_SURFACE_WG, bx, bz);
            if (gy <= level.getMinBuildHeight() + 4 || gy >= level.getMaxBuildHeight() - 14) {
                continue;
            }
        ...
        for (int s = 0; s < spots.length; ++s) {
            if (doorPts[s] == null || doorPts[s].length != 2 || doorPts[s][0] == 0 && doorPts[s][1] == 0) {
                continue;
            }
        ...
        for (int x = x0; x <= x1; ++x) {
            for (int z = z0; z <= z1; ++z) {
                ...
                for (int fy = Math.min(g, y0) - 1; fy <= y0; ++fy) {
                    DifuVillageFeature.set(l, p.set(x, fy, z), BRICK);
                }
                for (int dy = 1; dy <= h + 2; ++dy) {
                    DifuVillageFeature.clear(l, p.set(x, y0 + dy, z));
                }
                boolean edge = x == x0 || x == x1 || z == z0 || z == z1;
                corner = (x == x0 || x == x1) && (z == z0 || z == z1);
                if (edge) {
                    for (int dy = 1; dy <= h; ++dy) {
                        DifuVillageFeature.set(l, p.set(x, y0 + dy, z), corner ? CHISELED : BRICK);
                    }
                }
                DifuVillageFeature.set(l, p.set(x, y0 + h + 1, z), roof);
            }
        }
        int doorX = x0 + w / 2;
        DifuVillageFeature.clear(l, p.set(doorX, y0 + 1, z0));
        DifuVillageFeature.clear(l, p.set(doorX, y0 + 2, z0));
        DifuVillageFeature.clear(l, p.set(x1, y0 + 2, z0 + d / 2));
        DifuVillageFeature.clear(l, p.set(x0, y0 + 2, z0 + d / 2));
        DifuVillageFeature.set(l, p.set(x0 + 1, y0 + 1, z0 + 1), SOUL_TORCH);
        DifuVillageFeature.set(l, p.set(x1 - 1, y0 + 1, z1 - 1), SOUL_TORCH);
    ...
    private void tower(WorldGenLevel l, BlockPos.MutableBlockPos p, int x0, int y0, int z0) {
        ...
        for (int x = x0; x <= x1; ++x) {
            for (int z = z0; z <= z1; ++z) {
                ...
                if (edge) {
                    for (int dy = 1; dy <= h; ++dy) {
                        DifuVillageFeature.set(l, p.set(x, y0 + dy, z), corner ? CRACKED : BRICK);
                    }
                    DifuVillageFeature.set(l, p.set(x, y0 + h + 1, z), FENCE);
                }
                DifuVillageFeature.set(l, p.set(x, y0 + h, z), x == x0 + 1 || x == x0 + 2 ? SLAB : BRICK);
            }
        }
        DifuVillageFeature.clear(l, p.set(x0 + 1, y0 + 1, z0));
        DifuVillageFeature.clear(l, p.set(x0 + 1, y0 + 2, z0));
        DifuVillageFeature.set(l, p.set(x0 + 1, y0 + h, z0 + 1), SOUL_TORCH);
        DifuVillageFeature.set(l, p.set(x0 + 1, y0 + 1, z0 + 1), SOUL_TORCH);
        this.lampPost(l, p, x0 + 1, y0, z0 - 1);
    }

    private void lampPost(WorldGenLevel l, BlockPos.MutableBlockPos p, int x, int y, int z) {
        DifuVillageFeature.set(l, p.set(x, y, z), BRICK);
        DifuVillageFeature.set(l, p.set(x, y + 1, z), FENCE);
        DifuVillageFeature.set(l, p.set(x, y + 2, z), FENCE);
        DifuVillageFeature.set(l, p.set(x, y + 3, z), SOUL_TORCH);
    }
    ...
    private void paveAt(WorldGenLevel l, BlockPos.MutableBlockPos p, int x, int z) {
        ...
        BlockState g = l.getBlockState(p.set(x, top, z));
        if (g.is(Blocks.NETHERRACK) || g.is(Blocks.SOUL_SAND) || g.is(Blocks.SOUL_SOIL) || g.is(Blocks.NETHER_BRICKS)) {
            DifuVillageFeature.set(l, p.set(x, top, z), ROAD);
            DifuVillageFeature.clear(l, p.set(x, top + 1, z));
        }
    }

    private void spawnReaper(WorldGenLevel level, int x, int y, int z) {
        WanderingCultivatorEntity reaper = (WanderingCultivatorEntity) ((EntityType) ModEntities.WANDERING_CULTIVATOR.get()).create(level.getLevel());
        if (reaper == null) {
            return;
        }
