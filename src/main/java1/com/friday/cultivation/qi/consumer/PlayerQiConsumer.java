...
/**
 * 玩家灵气消费方 - 玩家作为 IQiConsumer 接收灵气。
 * 严格 1:1 复刻原 mod: xiaoxiang.cultivation.cultivation.qi.consumer.PlayerQiConsumer
 */
public final class PlayerQiConsumer implements IQiConsumer {
    ...
    public double attractRadius() {
        double r = BASE_ATTRACTION_RADIUS + (double) TechniqueBonusHelper.qiAbsorbRangeBonus(this.player);
        if (this.data.isMeditating()) {
            r += MEDITATION_RANGE_BONUS;
        }
        return r;
    }
    ...
    public int receiveQi(QiElement element, int baseAmount) {
        int amount = PlayerQiConsumer.finalAbsorbAmount(this.player, this.data, element, baseAmount);
        if (amount <= 0) {
            return 0;
        }
        ...
        if (qiGained > 0 || cultivationGained > 0L) {
            ...
            ModNetwork.CHANNEL.send(PacketDistributor.PLAYER.with(() -> this.player), new QiAbsorbedPacket(visualPos.x, visualPos.y, visualPos.z, element.ordinal()));
            ModNetwork.CHANNEL.send(PacketDistributor.PLAYER.with(() -> this.player), new SyncCultivationDataPacket(this.data));
        }
    ...
    public static int nominalQiRecoveryPerSecond(Player player, CultivationData data, QiElement element) {
        ...
        recovery = RealmPressureHandler.applyQiRecoveryPenalty(player, recovery);
    ...
    public static double finalAbsorbMultiplier(Player player, CultivationData data, QiElement element) {
        ...
        if (data != null && data.isMeditating()) {
            mult += MEDITATION_EFFICIENCY_BONUS;
        }
        return mult;
    }
