...
public class CultivatorFlightCombatGoal extends Goal {
    ...
    public void tick() {
        ...
        if (this.dodgeTicksRemaining > 0) {
            ...
            double dx = (this.npc.getRandom().nextDouble() - 0.5) * 1.0;
            double dy = (this.npc.getRandom().nextDouble() - 0.5) * 0.8;
            double dz = (this.npc.getRandom().nextDouble() - 0.5) * 1.0;
        ...
        double dist;
        double dx = cx - this.npc.getX();
        double dy = cy - this.npc.getY();
        double dz = cz - this.npc.getZ();
        Vec3 motion;
        if ((dist = Math.sqrt(dx * dx + dy * dy + dz * dz)) < 0.15) {
            double yWave = Math.sin(this.npc.tickCount / 10.0) * 0.02;
    ...
    private double computeCombatY(LivingEntity target) {
        ...
        boolean targetIsFlying;
        if (target.isNoGravity() || target.isFallFlying()) {
            targetIsFlying = true;
        } else if (target instanceof Player) {
            targetIsFlying = ((Player) target).getAbilities().mayfly;
        } else {
