...
/**
 * 蘑菇云视觉实体（爆炸后渲染用），noPhysics、不碰撞，纯粒子表现。
 * 严格 1:1 复刻原 mod MushroomCloudEntity。
 */
public class MushroomCloudEntity extends Entity {
    ...
    public MushroomCloudEntity(ServerLevel level, Vec3 center) {
        this((EntityType<? extends MushroomCloudEntity>)(EntityType<?>) ModEntities.MUSHROOM_CLOUD.get(), level);
    ...
    public void tick() {
        ...
        if (this.age < 400) {
            ...
            for (int i = 0; i < coreCount; ++i) {
                ...
                double r = this.random.nextDouble() * 6.0;
                double y = this.random.nextDouble() * 15.0;
        ...
        for (int i = 0; i < stemCount; ++i) {
            double angle = this.random.nextDouble() * Math.PI * 2.0;
            double r = this.random.nextDouble() * 10.0;
            double y = this.random.nextDouble() * 45.0;
            double px = cx + Math.cos(angle) * r;
            double pz = cz + Math.sin(angle) * r;
