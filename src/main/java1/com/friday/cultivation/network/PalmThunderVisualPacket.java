...
/**
 * 掌雷视觉同步包（服务端→客户端）— 严格复刻自原模组
 * com.xiaoxiang.cultivation.network.PalmThunderVisualPacket
 * <p>
 * 2 个 mode：
 * <ul>
 *   <li>CHANNEL (0) — 蓄力球</li>
...
public class PalmThunderVisualPacket {
    ...
    public static PalmThunderVisualPacket channel(int casterId, int durationTicks, boolean active) {
        return channel(casterId, durationTicks, active, active ? 1.0f : 0.0f, active);
    }
    ...
    public static void handle(PalmThunderVisualPacket msg, Supplier<NetworkEvent.Context> ctxSup) {
        ...
        ctx.enqueueWork(() -> DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> PalmThunderVisualHandler.onSync(msg.mode, msg.entityId, msg.durationTicks, msg.active, msg.x, msg.y, msg.z, msg.radius, msg.progress, msg.armed)));
