    ...
    public static void handle(FireSwordAuraPacket msg, Supplier<NetworkEvent.Context> ctxSup) {
        ...
        ctx.enqueueWork(() -> {
            ...
            CultivationCapability.get(player).ifPresent(data -> {
                ...
                long cost = TechniqueBonusHelper.applySpellQiCostMultiplier(player, Spell.SWORD_AURA, 50L);
                if (data.getCurrentQi() < cost) {
                    return;
                }
                data.setCurrentQi(data.getCurrentQi() - cost);
                CapabilityEvents.syncToClient(player);
                QiElement el = com.friday.cultivation.QiElement.byId(data.getDominantElement().id());
